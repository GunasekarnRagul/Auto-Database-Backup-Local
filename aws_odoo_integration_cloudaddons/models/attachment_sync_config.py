...
class AttachmentSyncConfig(models.Model):
    ...
    nextcloud_id = fields.Many2one('nextcloud.config', string='AWS Storage Connector', required=True, ondelete='cascade')
    ...
    def _create_attachment_categories(self):
        """Create folder structure for attachment categorization in AWS S3."""
        self.ensure_one()
        ...
        # In a real implementation, this would create subfolders in AWS S3
    ...
    def action_manual_drive_sync(self):
        ...
        if not self.nextcloud_id:
            raise UserError(
                "⚠️ No AWS S3 Storage selected. Please choose an AWS S3 account "
            ...
            return {
                ...
                'params': {
                    ...
                    'message': (
                        ...
                        'No files will be uploaded to AWS S3. '
        ...
        if not attachments:
            ...
            return {
                ...
                'params': {
                    ...
                    'message': (
                        f'All {model_label} files are already synced to AWS S3.\n'
        ...
        if synced_count > 0:
            message = (
                ...
                f"{synced_count} {model_label} file(s) synced to AWS S3.\n"
                f"Storage Mode: {mode_label} | Drive: {self.nextcloud_id.name}"
            )
            if failed_count:
    ...
    def action_setup_auto_sync(self):
        """Setup auto-sync: push all unsynced historical attachments to AWS S3.
        ...
        model_label = self.module_config_id.model_label or self.model_name
        message = f'Auto-sync setup complete. {synced_count} historical file(s) moved to AWS S3 for {model_label}.'
        if failed_count:
    ...
    def action_apply_sync(self):
        """Push all unsynced historical attachments to AWS S3.
