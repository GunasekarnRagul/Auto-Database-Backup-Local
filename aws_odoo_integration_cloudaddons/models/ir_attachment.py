...
class IrAttachment(models.Model):
    ...
    nextcloud_file_id = fields.Char('AWS S3 File ID', index=True)
    ...
    is_nextcloud_synced = fields.Boolean('AWS S3 Synced', compute='_compute_is_nextcloud_synced', store=True)
    ...
    def get_model_config(self):
        """Get the AWS S3 model configuration for this attachment."""
        self.ensure_one()
        ...
        for attachment in attachments:
            ...
            if res_model == 'mail.message' and res_id:
                ...
                if not attachment.datas and not attachment.raw:
                    import re
                    # Auto-recover if Odoo already copied the AWS S3 URL into the type='url' field
                    if attachment.type == 'url' and attachment.url and ('nextcloud.com' in attachment.url or 'nextcloud.com' in attachment.url):
                        # For AWS S3, the ID is often in the resid parameter
                        import urllib.parse as urlparse
                        # For AWS S3, extract the stable ID from the URL if stored
                        ...
                        if res_id_param:
                            ...
                            if config.storage_mode == 'drive':
                                attachment._inject_chatter_drive_button(
                                    attachment.url,
    ...
    def _sanitize_folder_name(name):
        """Strip/replace characters that are invalid or problematic in AWS S3
        WebDAV folder paths so that MKCOL / PROPFIND requests succeed.

        Characters removed or replaced:
          '  (apostrophe)     → removed  (causes HTTP 400 on some AWS S3 versions)
    ...
    def _inject_chatter_drive_button(self, drive_url, file_name=None, unlink_card=True, nextcloud_cfg=None, s3_key=None):
        """Single source-of-truth for chatter Drive-button injection.

        Generates a fresh presigned URL for private S3 buckets.
        Falls back to drive_url if presigned URL generation fails.

        Args:
            s3_key: Optional S3 object key (path) to use directly for presigning.
                    Pass this from _auto_sync_to_drive to avoid relying on
                    self.nextcloud_file_id which may not be refreshed in memory yet.
        """
        self.ensure_one()
        ...
        # ── Generate a presigned URL so the chatter link works on private S3 buckets ──
        display_url = drive_url  # fallback
        try:
            sync = self.env['nextcloud.sync'].sudo()
            # Resolve the config if not passed
            cfg = nextcloud_cfg
            if not cfg and self.nextcloud_file_id:
                file_rec = self.env['nextcloud.file'].sudo().search(
                    [('nextcloud_file_id', '=', self.nextcloud_file_id)], limit=1
                )
                cfg = file_rec.drive_config_id if file_rec else None
            if not cfg:
                cfg = self.env['nextcloud.config'].sudo().search(
                    [('active', '=', True), ('is_connected', '=', True)], limit=1
                ) or self.env['nextcloud.config'].sudo().search(
                    [('active', '=', True)], limit=1
                )
            if cfg:
                # Prefer the explicitly-passed s3_key (set by _auto_sync_to_drive from the
                # upload result) so we never fall back to a stale self.nextcloud_file_id
                # that hasn't been refreshed in memory yet — that caused auto-sync to use
                # the full plain S3 URL as the key, returning '' from _get_share_link.
                resolved_key = s3_key or sync._get_remote_path_from_id(self.nextcloud_file_id or drive_url)
                presigned = sync._get_share_link(resolved_key, cfg)
                if presigned:
                    display_url = presigned
        except Exception as e:
            _logger.warning('_inject_chatter_drive_button: could not generate presigned URL: %s', e)
        ...
        btn_html = (
            ...
            "<b><i class='fa fa-cloud'></i> AWS S3 Documents:</b>"
            "<ul class='list-unstyled mb-0 mt-2'>"
            f"<li><a href='{display_url}' target='_blank' rel='noopener noreferrer' "
        ...
        for msg in linked_messages:
            ...
            current_body = msg.body or ''
            if display_url not in current_body:
                if drive_url in current_body and drive_url != display_url:
                    # Upgrade the old plain link to the secure presigned link
                    write_vals['body'] = Markup(current_body.replace(drive_url, display_url))
                else:
                    # Append new button entirely
                    write_vals['body'] = Markup(current_body) + Markup(btn_html)
            msg.with_context(
        ...
        if any(f in vals for f in trigger_fields):
            for attachment in self:
                ...
                if res_model == 'mail.message' and res_id:
                    ...
                    if not attachment.datas and not attachment.raw:
                        import re
                        # Auto-recover if Odoo already copied the AWS S3 URL into type='url'
                        if attachment.type == 'url' and attachment.url and ('nextcloud.com' in attachment.url or 'nextcloud.com' in attachment.url):
                            import urllib.parse as urlparse
    ...
    def _auto_sync_to_drive(self, config):
        ...
        if not nextcloud_cfg:
            _logger.warning("_auto_sync_to_drive: no active AWS S3 config found, aborting.")
            return
        ...
        # If parent_id is still a numeric string (AWS S3 file ID), resolve it to a path
        if parent_id and not str(parent_id).startswith('/'):
            parent_id = sync_service._get_remote_path_from_id(parent_id)

        # ── Ensure the config base folder exists on AWS S3 ─────────────────────
        # The folder may exist in Odoo's DB but not yet on the AWS S3 server.
        ...
        try:
            ...
            if result:
                # ── Generate presigned URL immediately after upload ───────────────────────
                # Both the stored att.url AND the chatter button must use a presigned URL.
                # The plain _s3_format_url() result is always Access Denied on private buckets.
                s3_key_path = result['nextcloud_file_id']   # e.g. /ragul/attach/CRM/.../9.png
                plain_url   = result['nextcloud_url']        # plain S3 URL (fallback only)
                try:
                    presigned_url = sync_service._get_share_link(s3_key_path, nextcloud_cfg)
                    link_url = presigned_url if presigned_url else plain_url
                except Exception as _pse:
                    _logger.warning('_auto_sync_to_drive: presigned URL failed, using plain URL: %s', _pse)
                    link_url = plain_url

                # Write Drive file ID + metadata back to this attachment
                att_vals = {
                    'nextcloud_file_id': s3_key_path,
                ...
                # Store the presigned URL (not the plain S3 URL) so the attachment link works.
                if storage_mode == 'drive':
                    att_vals.update({
                        'type': 'url',
                        'url': link_url,       # ← presigned URL stored here
                        'datas': False,
                        'db_datas': False,
                        'store_fname': False,
                    })
                ...
                # Pass the already-computed link_url (presigned) and s3_key_path directly
                # so _inject_chatter_drive_button doesn't need to re-generate the URL.
                self._inject_chatter_drive_button(
                    link_url,
                    nextcloud_cfg=nextcloud_cfg,
                    ...
                    if res_model_target and res_id_target \
                        ...
                        if messages:
                            ...
                            for ghost in ghost_atts:
                                ...
                                ghost._inject_chatter_drive_button(
                                    synced_url,
                                    nextcloud_cfg=nextcloud_cfg
                                )
                # ────────────────────────────────────────────────────────────────────────
    ...
    def _get_sync_subfolder_name(self, model, res_id):
        """Determine the 2-level folder structure for AWS S3 sync.
    ...
    def action_sync_to_drive(self):
        """Manually sync an attachment to AWS S3 using the model's sync configuration.
        ...
        if config:
            ...
            return {
                ...
                'params': {
                    'title': _('Success'),
                    'message': _('File "%s" uploaded to AWS S3 using the %s configuration.') % (self.name, config.name),
                    'type': 'success',
                    'sticky': False,
            ...
            if not self.nextcloud_id:
                ...
                if active_drives:
                    ...
                    raise UserError(_("Please select a AWS S3 first or ensure at least one AWS S3 is configured and active."))
            ...
            try:
                ...
                if result:
                    ...
                    return {
                        ...
                        'params': {
                            'title': _('Success'),
                            'message': _('File "%s" uploaded to AWS S3 successfully.') % self.name,
                            'type': 'success',
                            'sticky': False,
                        }
                    }
                else:
                    raise UserError(_("Failed to upload file to AWS S3."))
            except Exception as e:
