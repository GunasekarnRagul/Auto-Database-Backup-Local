    ...
    def _process_drive_only_attachments(self, messages):
        ...
        for msg in messages:
            ...
            for att in msg.attachment_ids:
                if att.type == 'url' and att.nextcloud_file_id and att.url:
                    # Generate a fresh presigned URL using the stored S3 key.
                    # att.url may be an expired or plain S3 URL — always re-sign.
                    display_url = att.url  # fallback
                    try:
                        ...
                        if cfg:
                            s3_key = att.nextcloud_file_id  # already a path like /ragul/attach/...
                            fresh = sync._get_share_link(s3_key, cfg)
                            if fresh:
                                display_url = fresh
                    except Exception:
                        pass
                    drive_links.append((att.name, display_url))
            ...
            if drive_links:
                ...
                body_parts.append("<b><i class='fa fa-cloud'></i> AWS S3 Documents:</b>")
