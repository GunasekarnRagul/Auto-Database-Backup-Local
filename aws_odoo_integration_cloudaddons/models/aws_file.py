...
class NextcloudFile(models.Model):
    ...
    _description = 'AWS S3 File'
    _order = 'file_type desc, name asc'

    _sql_constraints = [
        ('nextcloud_file_id_drive_config_unique', 'unique(nextcloud_file_id, drive_config_id)',
         'A file with this Google ID already exists for this drive configuration.')
    ]

    name = fields.Char('File Name', required=True)
    drive_config_id = fields.Many2one('nextcloud.config', string='Drive', required=True, ondelete='cascade')
    root_folder_id = fields.Many2one('nextcloud.root.folder', string='Root Folder', ondelete='cascade')
    ...
    md5_checksum = fields.Char('MD5 Checksum', index=True, help="AWS S3 MD5 hash for exact content matching")
    ...
    nextcloud_file_id = fields.Char('AWS S3 File ID')
    nextcloud_url = fields.Char('AWS S3 URL')
    parent_folder_id = fields.Many2one('nextcloud.file', string='Parent Folder',
                                       domain="[('file_type', '=', 'folder')]")
    child_ids = fields.One2many('nextcloud.file', 'parent_folder_id', string='Contents')
    ...
    res_id = fields.Many2oneReference('Resource ID', compute='_compute_res_model_id', model_field='res_model', store=True, index=True, readonly=False, recursive=True)
    ...
    def _compute_attachment_id(self):
        for record in self:
            if record.nextcloud_file_id:
                attachment = self.env['ir.attachment'].sudo().search([
                    ('nextcloud_file_id', '=', record.nextcloud_file_id)
                ], limit=1)
    ...
    def _resolve_parent_nextcloud_id(self, parent_folder_id=False, root_folder_id=False):
        """Resolve the AWS S3 parent folder ID (remote path) from Odoo records.

        Returns the remote path string (e.g. '/MyFolder') that the sync engine
        can use to construct S3 keys.  Falls back to '/<folder_name>' when the
        root folder has not yet been assigned a remote path (root_id is empty).
        """
        if parent_folder_id:
            parent_rec = self.browse(parent_folder_id)
            if parent_rec.exists() and parent_rec.nextcloud_file_id:
                return parent_rec.nextcloud_file_id
        if root_folder_id:
            root_rec = self.env['nextcloud.root.folder'].browse(root_folder_id)
            if root_rec.exists():
                # root_id may be empty for brand-new root folders not yet synced
                return root_rec.root_id or ('/' + root_rec.name)
        return False
    ...
    def unlink(self):
        ...
        return super(NextcloudFile, self).unlink()
    ...
    def action_prompt_delete(self):
        ...
        configs = self.env['attachment.sync.config'].search([('nextcloud_folder_id', '=', self.id)])
        
        return {
            'name': 'Confirm Deletion',
            'type': 'ir.actions.act_window',
            'res_model': 'nextcloud.file.delete.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_archive_recursive(self):
        """Archive records locally only. No changes made to AWS S3."""
        for record in self:
    ...
    def action_unarchive(self):
        """Restore archived records locally. No changes made to AWS S3."""
    ...
    def delete_on_drive_and_unlink(self, record_ids):
        """Move records to AWS S3 Trash and then unlink them from Odoo.
        ...
        for record in records:
            ...
            if not record.nextcloud_file_id:
                success_ids.append(record.id)
                continue
            
            # Use trash_file instead of hard delete per user request
            if sync.trash_file(record.nextcloud_file_id, record.drive_config_id):
                success_ids.append(record.id)
        ...
        if success_ids:
            ...
            all_to_unlink = self.env['nextcloud.file']
    ...
    def action_move_items(self, item_ids, target_parent_id=None, target_root_id=None):
        ...
        if target_parent_id:
            ...
            # Force sync the target folder if it's pending, so it exists on AWS S3
            ...
            try:
                ...
                if success:
                    ...
                    log_model.log_operation(
                        ...
                        operation='move',
                        state='fail',
                        error_message='Move operation failed on AWS S3.',
                        sync_type='manual',
    ...
    def get_trash_roots(self):
        ...
        # Find the roots
        trash_roots = self.env['nextcloud.file']
        ...
        result = trash_roots.read([
            "name", "file_type", "mime_type", "nextcloud_url", "file_size",
            "owner_name", "last_modified", "sync_state", "starred", 
            "drive_config_id", "nextcloud_file_id", "attachment_id", "display_path"
        ])
        return result

    @api.model
    def rename_on_drive_by_id(self, record_id, new_name, old_name=None):
        """Rename a file/folder on AWS S3. 
        Called asynchronously from JS after Odoo UI updates instantly.
        """
        record = self.browse(record_id)
        if not record.exists() or not record.nextcloud_file_id:
            return False
        ...
        try:
            ...
            if success:
                ...
                log_model.log_operation(
                    ...
                    operation='rename',
                    state='fail',
                    error_message='Rename operation failed on AWS S3.',
                    sync_type='manual',
    ...
    def action_upload_from_explorer(self, file_name, file_data, mime_type,
        ...
        Creates a local nextcloud.file record with 'pending' status.
        The actual upload to AWS S3 happens when the user clicks Sync.
        """
        config = self.env['nextcloud.config'].browse(drive_config_id)
        ...
        attachment = self.env['ir.attachment'].with_context(skip_nextcloud_sync=True).create({
            'name': file_name,
            'raw': raw_data,
            'mimetype': mime_type,
    ...
    def _sync_single_record(self, record):
        ...
        sync = self.env['nextcloud.sync'].sudo()
        log_model = self.env['nextcloud.sync.log']
        ...
        folder_path = False
        if record.root_folder_id:
            root_rec = self.env['nextcloud.root.folder'].sudo().browse(record.root_folder_id.id)
        ...
        # 1. New Folder
        if record.file_type == 'folder' and not record.nextcloud_file_id:
            parent_nextcloud_id = record._resolve_parent_nextcloud_id(
        ...
        # 2. New File
        if record.file_type == 'file' and not record.nextcloud_file_id:
            parent_nextcloud_id = record._resolve_parent_nextcloud_id(
                record.parent_folder_id.id if record.parent_folder_id else False,
                record.root_folder_id.id if record.root_folder_id else False,
            )
            ...
            try:
                ...
                result = sync.with_context(sync_type=sync_type).upload_file_to_drive(
                    ...
                    record.drive_config_id, parent_nextcloud_id=parent_nextcloud_id,
                    file_record=record
                )
        ...
        if record.nextcloud_file_id:
            try:
                success = sync.rename_file(record.nextcloud_file_id, record.name, record.drive_config_id)
                if success:
                    record.write({
                        'sync_state': 'synced',
                        'last_synced': fields.Datetime.now(),
    ...
    def action_get_share_info(self):
        ...
        self.ensure_one()
        if not self.nextcloud_file_id:
            return {'error': 'File not yet synced to AWS S3.'}

        # ── Folders cannot be shared via presigned URL ──────────────────────────
        # In AWS S3, folders are virtual prefixes (not real objects).
        # A presigned URL on a folder marker returns NoSuchKey or downloads
        # an empty 0-byte object — neither is useful for sharing.
        ...
        sync = self.env['nextcloud.sync'].sudo()
        try:
            perms = sync.get_file_permissions(self.nextcloud_file_id, self.drive_config_id)
            # Always generate a fresh presigned URL — the plain S3 URL stored in nextcloud_url
            # returns AccessDenied on private buckets.
            presigned_url = sync._get_share_link(
                sync._get_remote_path_from_id(self.nextcloud_file_id), self.drive_config_id
            )
            expiry_days = max(1, min(7, int(self.drive_config_id.share_link_expiry_days or 7)))
            links = []
            if presigned_url:
                links = [{
                    'id': 'public_link',
                    'url': presigned_url,
                    'scope': 'anonymous',
                    'role': self.anyone_role or 'reader',
                    'description': f'Anyone with the link can view (valid for {expiry_days} day{"s" if expiry_days != 1 else ""})',
                    'recipients': [],
                }]
            general = 'anyone' if presigned_url else 'restricted'
            return {
                'is_folder': False,
                'permissions': perms,
                'links': links,
                'shareLink': presigned_url or '',
    ...
    @api.model
    def action_get_bulk_share_links(self, file_ids):
        """Generate fresh presigned URLs for a list of file IDs.
        Called from the multi-file Share Drive Link dialog.
        Folders are skipped — AWS S3 does not support folder-level presigned URLs.
        Returns a list of: {id, name, url, is_folder, error}.
        """
        ...
        for rec in records:
            if rec.file_type == 'folder':
                result.append({
                    ...
                    'url': '',
                    'is_folder': True,
                    'error': 'Folder sharing is not supported in AWS S3.',
                })
                continue
            if not rec.nextcloud_file_id:
                result.append({'id': rec.id, 'name': rec.name, 'url': '', 'is_folder': False, 'error': 'Not synced'})
                continue
            try:
                url = sync._get_share_link(
                    sync._get_remote_path_from_id(rec.nextcloud_file_id),
                    rec.drive_config_id
                )
                result.append({'id': rec.id, 'name': rec.name, 'url': url or '', 'is_folder': False, 'error': ''})
            except Exception as e:
                result.append({'id': rec.id, 'name': rec.name, 'url': '', 'is_folder': False, 'error': str(e)})
        return result
    ...
    def action_add_permission(self, email, role='reader', send_notification=True):
        ...
        try:
            result = sync.create_permission(
                self.nextcloud_file_id, email, role, self.drive_config_id,
            ...
            if isinstance(result, dict):
                if result.get('ok'):
                    ...
                    return response
                return {'error': result.get('error', 'AWS S3 could not add the member.')}
            # Legacy: plain True/False
            if result:
                self.sudo().write({'shared_people_count': self.shared_people_count + 1})
                return {'success': True}
            return {'error': 'AWS S3 could not add the member. Check the email and try again.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_update_permission(self, permission_id, role):
        ...
        sync = self.env['nextcloud.sync'].sudo()
        try:
            ok = sync.update_permission(self.nextcloud_file_id, permission_id, role, self.drive_config_id)
            if ok:
                return {'success': True}
            return {'error': 'Could not update permission on AWS S3.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_remove_permission(self, permission_id):
        ...
        sync = self.env['nextcloud.sync'].sudo()
        try:
            ok = sync.delete_permission(self.nextcloud_file_id, permission_id, self.drive_config_id)
            if ok:
                return {'success': True}
            return {'error': 'Could not remove permission on AWS S3.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_set_general_access(self, access_type, role='reader', block_download=None, password=None, expirationDate=None):
        ...
        try:
            result = sync.set_general_access(
                self.nextcloud_file_id, access_type, role, self.drive_config_id,
            ...
            if result:
                self.sudo().write({'permission_type': access_type, 'anyone_role': role})
                return {'success': True}
            return {'error': 'Could not update access settings on AWS S3.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_generate_shareable_link(self, role='reader'):
        ...
        sync = self.env['nextcloud.sync'].sudo()
        try:
            res = sync.set_general_access(self.nextcloud_file_id, 'anyone', role, self.drive_config_id)
    ...
    def action_revoke_shareable_link(self):
        ...
        sync = self.env['nextcloud.sync'].sudo()
        try:
            res = sync.set_general_access(self.nextcloud_file_id, 'restricted', 'reader', self.drive_config_id)
            if res:
                return {'success': True}
    ...
    def get_duplicate_groups(self, config_id=None):
        ...
        Allows filtering by a specific AWS S3 config.
        """
    ...
    def prune_duplicates(self, keep_newest=True):
        ...
        files_to_delete = self.env['nextcloud.file']
