...
export class FileExplorer extends Component {
    setup() {
        ...
        if (this.busService) {
            ...
            this.busService.addChannel('nextcloud.sync');
            this.busService.subscribe('notification', (notifications) => {
                notifications.forEach(notif => {
                    if (notif.type === 'nextcloud.sync' && notif.payload.type === 'folder_synced') {
        ...
        onWillStart(async () => {
            ...
            if (section === 'trash') {
                ...
                if (this.state.currentFolderId) {
                    try {
                        const breadcrumbs = await this.orm.call("nextcloud.file", "get_folder_breadcrumbs", [this.state.currentFolderId]);
    ...
    get hasNoConnectedDrives() {
        // True when there IS an active drive selected but it is NOT connected
        if (this.state.drives.length === 0) return false;
        const active = this.state.drives.find(d => d.id === this.state.activeDriveId);
        return active ? active.state !== 'connected' : false;
    }
    ...
    async loadFiles(folderId = null) {
        ...
        try {
            if (section === 'starred') {
                files = await this.orm.searchRead("nextcloud.file",
                ...
                if (recentIds.length === 0) {
                    ...
                    const recentFiles = await this.orm.searchRead("nextcloud.file",
                        domain,
                        commonFields
                    );
                ...
                if (folderId === 'trash_root' || !folderId) {
                    files = await this.orm.call("nextcloud.file", "get_trash_roots", []);
                } else {
                    files = await this.orm.searchRead("nextcloud.file",
                ...
                files = await this.orm.searchRead("nextcloud.file", domain, commonFields);
            }
    ...
    getFileImageUrl(file, isPreview = false) {
        ...
        const mime = file.mime_type || '';
        const basePath = '/aws_odoo_integration_cloudaddons/static/description/';
    ...
    async executeSearch() {
        ...
        try {
            const files = await this.orm.searchRead("nextcloud.file", domain, [
                "name", "file_type", "mime_type", "nextcloud_url", "file_size",
                "owner_name", "last_modified", "sync_state", "upload_progress", "starred",
                "drive_config_id", "nextcloud_file_id", "attachment_id", "display_path",
                "parent_folder_id"
    ...
    onFileClick(ev, file) {
        ...
        if (file.file_type === 'folder') {
            ...
            // Extensions/mimes that browsers cannot preview — trigger download instead.
            const skipPreviewMimes = ['zip', 'x-rar', 'x-tar', 'archive', 'compressed', 'x-msdownload', 'x-executable', '7z'];
            const skipPreviewExts  = ['.zip', '.rar', '.7z', '.tar', '.gz', '.bz2', '.xz',
                                      '.exe', '.msi', '.dmg', '.pkg', '.deb', '.rpm',
                                      '.iso', '.img', '.bin'];
            const canSkipPreview = skipPreviewMimes.some(t => mime.includes(t))
                                || skipPreviewExts.some(e => name.endsWith(e));

            if (file.nextcloud_file_id && !canSkipPreview) {
                // Previewable — open in dialog with iframe
                this.dialogService.add(FilePreviewDialog, {
                    file: file,
                    onDownload: () => {
                        window.open(`/nextcloud/download/${file.id}`, '_blank');
                    },
                    onOpenDetails: () => {
            ...
            } else if (file.nextcloud_file_id) {
                // Not previewable (archive, executable, etc.) — download via Odoo
                // so it uses IAM credentials and never exposes a plain S3 URL.
                window.open(`/nextcloud/download/${file.id}`, '_blank');
            } else {
                // File not yet synced — open form to show status
                this.actionService.doAction({
                    type: 'ir.actions.act_window',
                    res_model: 'nextcloud.file',
                    res_id: file.id,
    ...
    async onPaste() {
        ...
        if (this.state.activeDriveId !== this.state.clipboard.sourceDriveId) {
            this.notificationService.add("Other drive not pasted. Moving items across different Nextclouds is not supported.", { 
                type: "danger",
                sticky: true 
            });
            return;
    ...
    async _loadTreeChildren(parentId, rootId = null) {
        ...
        try {
            ...
            const children = await this.orm.searchRead("nextcloud.file", domain, ["id", "name"]);
    ...
    async _updateNavigationState(folderId, folderName) {
        ...
        try {
            const breadcrumbs = await this.orm.call("nextcloud.file", "get_folder_breadcrumbs", [folderId]);
    ...
    async toggleStarSelected() {
        ...
        try {
            ...
            await this.orm.write("nextcloud.file", ids, { starred: newState });
    ...
    async toggleStar(ev, file) {
        ...
        try {
            await this.orm.write("nextcloud.file", [file.id], { starred: newState });
    ...
    async onRestoreSelected() {
        ...
        try {
            const ids = files.map(f => f.id);
            await this.orm.call("nextcloud.file", "action_unarchive", [ids]);
    ...
    async confirmPermanentDelete() {
        ...
        try {
            await this.orm.call("nextcloud.file", "delete_on_drive_and_unlink", [ids]);
            this.clearSelection();
            await this.onSidebarClick('trash');
    ...
    onDownloadSelected() {
        ...
        if (hasFolder || isMulti) {
            ...
            const downloadUrl = `/nextcloud/download_zip?file_ids=${ids}`;
        ...
        const file = files[0];
        const downloadUrl = `/nextcloud/download/${file.id}`;
    ...
    async onDeleteSelected() {
        ...
        try {
            ...
            await this.orm.call("nextcloud.file", "action_archive_recursive", [ids]);
            ...
            this.notificationService.add(
                `Moved to Odoo Trash: ${nameList}. These items will remain active on Nextcloud until permanently deleted from the Trash tab.`,
                { type: "info" }
            );
            await this.checkPendingChanges();
        } catch (e) {
    ...
    async confirmRename(file) {
        ...
        try {
            ...
            await this.orm.write("nextcloud.file", [fileId], {
                name: newName,
                sync_state: 'pending'
            });
            this.clearSelection();
            ...
            if (file.nextcloud_file_id) {
                try {
                    await this.orm.call("nextcloud.file", "rename_on_drive_by_id", [], {
                        record_id: fileId,
                        new_name: newName,
                        old_name: file.name,
                    });
                    this.loadFiles(this.state.currentFolderId); // Refresh to clear 'pending'
                    this.notificationService.add(`Renamed to "${newName}" on Nextcloud`, { type: "success" });
                } catch (e) {
                    this.notificationService.add(`Failed to rename "${newName}" on Nextcloud`, { type: "warning" });
                }
            } else {
    ...
    async checkPendingChanges() {
        if (!this.state.activeDriveId) return;
        const count = await this.orm.searchCount("nextcloud.file", [
    ...
    async confirmCreateFolder() {
        ...
        try {
            await this.orm.create("nextcloud.file", [{
                name: name,
                file_type: 'folder',
    ...
    async _uploadSingleFile(file, targetFolderId, targetRootId) {
        ...
        await this.orm.call("nextcloud.file", "action_upload_from_explorer", [], {
            file_name: file.name,
            file_data: base64,
    ...
    async onManualSync() {
        ...
        try {
            ...
            await this.orm.call("nextcloud.config", "action_trigger_sync", [], {
    ...
    /**
     * Trigger a one-way auto-sync (Odoo → Nextcloud) for a specific drive.
     *
     ...
     *
     * @param {number|null} driveId  nextcloud.config ID to sync.
                ...
                for (const id of pendingIds) {
                    try {
                        ...
                        if (this.state.activeDriveId === targetDriveId) {
                            const updated = await this.orm.read("nextcloud.file", [id], [
                                "sync_state", "nextcloud_file_id", "nextcloud_url", "last_synced", "upload_progress"
                            ]);
...
FileExplorer.template = "nextcloud_odoo_integration.FileExplorer";

registry.category("actions").add("nextcloud_odoo_integration.file_explorer", FileExplorer);

class FilePreviewDialog extends Component {
    static template = "nextcloud_odoo_integration.FilePreviewDialog";
    ...
    async loadPreviewUrl() {
        ...
        if (file.nextcloud_file_id) {
            try {
                ...
                const url = await this.orm.call("nextcloud.sync", "get_preview_url", [
                    file.nextcloud_file_id, 
                    configId
                ]);
                if (url) {
                    ...
                    this.state.error = "AWS S3 could not generate a preview for this file type.";
                }
            } catch (e) {
    ...
    get openInDriveUrl() {
        // /nextcloud/open/<id> generates an S3 presigned URL and redirects
        // the browser to it — the file opens directly in S3 (inline for
        // images/PDFs, native download for other types).
        const { file } = this.props;
        return `/nextcloud/open/${file.id}`;
    }
}

export class ShareDriveLinkDialog extends Component {
    static template = "nextcloud_odoo_integration.ShareDriveLinkDialog";
    static components = { Dialog };
    static props = {
        files: Array,
        close: Function,
    ...
    setup() {
        ...
        this.state = useState({
            ...
            justSynced: false,
            lastSyncTime: null,
            // Multi-file bulk presigned links: { [fileId]: presignedUrl }
            bulkShareLinks: {},
            // Set to true when the single selected item is a folder
            isFolder: false,
        });
        ...
        onWillStart(async () => {
            if (this.props.files.length === 1 && this.props.files[0].nextcloud_file_id) {
                // Single file: load full share info with presigned URL
                await this.loadShareInfo();
            } else if (this.props.files.length > 1) {
                // Multiple files: generate a presigned URL for each file
                await this.loadBulkShareLinks();
            } else {
                this.state.loading = false;
            }
    ...
    // ─── Multi-File Bulk Links ───

    async loadBulkShareLinks() {
        this.state.loading = true;
        this.state.error = null;
        try {
            const fileIds = this.props.files
                .filter(f => f.nextcloud_file_id)
                .map(f => f.id);
            if (fileIds.length === 0) {
                this.state.loading = false;
                return;
            ...
            const results = await this.orm.call(
                'nextcloud.file',
                'action_get_bulk_share_links',
                [],
                { file_ids: fileIds }
            );
            // Build lookup: { fileId -> presignedUrl }
            const linkMap = {};
            for (const item of (results || [])) {
                linkMap[item.id] = item.url || '';
            }
            this.state.bulkShareLinks = linkMap;
        } catch (e) {
            console.error('Failed to load bulk share links', e);
        } finally {
            this.state.loading = false;
        }
    }

    /** Files that have an S3 key, enriched with a presigned url from state.bulkShareLinks */
    get shareableFilesWithLinks() {
        return this.props.files
            .filter(f => f.nextcloud_file_id || f.file_type === 'folder')
            .map(f => ({
                ...f,
                presignedUrl: this.state.bulkShareLinks[f.id] || '',
                is_folder: f.file_type === 'folder',
            }));
    }
    ...
    async refreshPermissions() {
        if (this.state.isSyncing || !this.isSingleFile || !this.file.nextcloud_file_id) return;
    ...
    _startAutoSync() {
        ...
        // Poll Nextcloud every 30s while the dialog is open
    ...
    get shareableFiles() {
        return this.props.files.filter(f => f.nextcloud_url);
    }

    get shareLink() {
        const file = this.file;
        if (!file || !file.nextcloud_file_id) return '';
        // Prefer the presigned URL from the backend (state.shareLink) over
        // the plain S3 URL (nextcloud_url) which returns AccessDenied on private buckets.
        return this.state.shareLink || file.nextcloud_url || '';
    }
    ...
    async loadShareInfo() {
        ...
        try {
            ...
            if (result.error) {
                ...
                // Folder selected — S3 folder sharing not supported
                this.state.isFolder = true;
            } else {
                this.state.isFolder = false;
                this.state.permissions = result.permissions || [];
                this.state.links = result.links || [];
                this.state.shareLink = result.shareLink || '';
                this.state.generalAccess = result.generalAccess || 'restricted';
                this.state.anyoneRole = result.anyoneRole || 'reader';
                this.state.writersCanShare = result.writersCanShare !== false;
                this.state.copyRequiresWriterPermission = result.copyRequiresWriterPermission || false;
                const now = new Date();
                this.state.lastSyncTime = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                // Start auto-polling so changes reflect here every 30s
                this._startAutoSync();
            }
        } catch (e) {
    ...
    async addPerson() {
        ...
        try {
            ...
            if (result.error) {
                ...
                // If Nextcloud used a shared link instead of a direct invite,
                // update the file URL so "Copy link" always reflects the new link.
                if (result.fallback_link) {
                    this.file.nextcloud_url = result.fallback_link;
                }
    ...
    onImageError(ev, perm) {
        ...
        if (avatarDiv) {
            const initialsSpan = avatarDiv.querySelector('.od_avatar_initial');
            if (initialsSpan) {
                initialsSpan.classList.remove('od_hidden');
            }
        }
    }
        ...
        try {
            ...
            if (result.error) {
                ...
                if (result.link && result.link.webUrl) {
                    this.file.nextcloud_url = result.link.webUrl;
                }
    ...
    async onCopyLink() {
        const link = this.shareLink || this.shareableFiles.map(f => f.nextcloud_url).join('\n');
        if (!link) return;
        try {
