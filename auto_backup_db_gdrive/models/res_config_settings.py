...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        # Explicitly save retention policy fields to ensure they are available in cron
        ICP.set_param('auto_backup_db_gdrive.retention_type', self.gdrive_retention_type)
        ICP.set_param('auto_backup_db_gdrive.retention_count', self.gdrive_retention_count)
        ICP.set_param('auto_backup_db_gdrive.retention_days', self.gdrive_retention_days)
    ...
    def get_values(self):
        ...
        # Retrieve retention settings explictly to ensure consistency
        retention_type = ICP.get_param('auto_backup_db_gdrive.retention_type', 'none')
        try:
            retention_count = int(ICP.get_param('auto_backup_db_gdrive.retention_count', '10'))
        except (ValueError, TypeError):
            retention_count = 10
        try:
            retention_days = int(ICP.get_param('auto_backup_db_gdrive.retention_days', '30'))
        except (ValueError, TypeError):
            retention_days = 30
        ...
        res.update(
            ...
            gdrive_retention_type=retention_type,
            gdrive_retention_count=retention_count,
            gdrive_retention_days=retention_days,
        )
        return res
    ...
    def action_manual_sync(self, backup_type='Manual'):
        ...
        try:
            if backup_type == 'Auto':
                sync_scope = self.gdrive_sync_scope or 'all'
                db_ids = self.gdrive_selected_db_ids
            else:
                sync_scope = self.gdrive_manual_sync_scope or 'all'
                db_ids = self.gdrive_manual_selected_db_ids

            if sync_scope == 'selective':
                db_list = db_ids.mapped('name')
                if not db_list:
                     return {
                        'type': 'ir.actions.client',
                        'tag': 'display_notification',
    ...
    def _cron_auto_backup(self):
        ...
        # Create a record populated with current settings to ensure retention policy works
        config = self.create(self.get_values())
    ...
    def _apply_retention_policy(self, access_token, backup_type):
        ...
        # 1. Safely retrieve the retention type (prefer self for manual sync unsaved changes)
        retention_type = getattr(self, 'gdrive_retention_type', False)
        if not retention_type or retention_type == 'none':
            # Fallback to ICP if not in self
            retention_type = ICP.get_param('auto_backup_db_gdrive.retention_type')
            
        if not retention_type or retention_type in ['none', 'False']:
            _logger.info("Retention policy is set to 'none'. Skipping cleanup.")
            return
            
        parent_id = getattr(self, 'gdrive_folder_id', False) or ICP.get_param('auto_backup_db_gdrive.folder_id')
        if not parent_id or parent_id == 'False':
            # Default to root if no folder ID is configured, instead of skipping
            _logger.info("No Google Drive Folder ID configured. Using 'root' for retention policy.")
            parent_id = 'root'
        ...
        _logger.info("Applying retention policy for %s: Total (Type: %s). Query: %s", backup_type, retention_type, query)
        ...
        if retention_type == 'count':
            try:
                retention_count = getattr(self, 'gdrive_retention_count', 0)
                ...
                _logger.info("Retention Count Policy: %s", retention_count)
            except (ValueError, TypeError):
                retention_count = 10
        ...
        # 4. Apply: "Keep for X Days"
            ...
            try:
                retention_days = getattr(self, 'gdrive_retention_days', 0)
