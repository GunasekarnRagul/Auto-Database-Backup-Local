...
class GoogleDriveFile(models.Model):
        ...
        try:
            ...
            if isinstance(result, dict):
                if result.get('ok'):
                    ...
                    return response
                return {'error': result.get('error', 'Dropbox could not add the member.')}
            # Legacy: plain True/False
            if result:
                self.sudo().write({'shared_people_count': self.shared_people_count + 1})
                return {'success': True}
            return {'error': 'Dropbox could not add the member. Check the email and try again.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_update_permission(self, permission_id, role):
        ...
        try:
            ...
            if ok:
                return {'success': True}
            return {'error': 'Could not update permission on Dropbox.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_remove_permission(self, permission_id):
        ...
        try:
            ...
            if ok:
                return {'success': True}
            return {'error': 'Could not remove permission on Dropbox.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_set_general_access(self, access_type, role='reader', block_download=None, password=None, expirationDate=None):
        ...
        try:
            ...
            if result:
                self.sudo().write({'permission_type': access_type, 'anyone_role': role})
                return {'success': True}
            return {'error': 'Could not update access settings on Dropbox.'}
        except Exception as e:
            return {'error': str(e)}
