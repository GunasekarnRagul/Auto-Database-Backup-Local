...
class GoogleDriveSyncLog(models.Model):
    ...
    def create_log(self, vals):
        ...
        """
        try:
            log_rec = self.sudo().create(vals)
            # Notify the UI to auto-refresh the logs menu for all users listening
            self.env['bus.bus']._sendone('one.drive.sync', 'one.drive.sync', {'type': 'sync_log_updated'})
            return log_rec
        except Exception:
