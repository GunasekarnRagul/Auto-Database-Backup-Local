...
class NextcloudFolderWizard(models.TransientModel):
    ...
    def action_fetch_folders(self):
        """Fetch top-level folders from the connected Nextcloud."""
        self.ensure_one()
        ...
        if not access_token:
            return {
                ...
                'params': {
                    ...
                    'message': _('Could not connect to Nextcloud. Please check your credentials.'),
                    'sticky': False,
                }
            }
        ...
        # Use Nextcloud list_folder to get top-level folders
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                _logger.warning("Nextcloud API error during folder fetch: %s", response.text)
...
class NextcloudFolderWizardLine(models.TransientModel):
    ...
    _description = 'Nextcloud Folder Bulk Add Wizard Line'
