...
class NextcloudDashboard(models.AbstractModel):
    _name = 'nextcloud.dashboard'
    _description = 'Nextcloud Sync Dashboard Data'
    ...
    def get_kpi_data(self):
        ...
        try:
            GDFile = self.env['nextcloud.file'].sudo()
            SyncConfig = self.env['attachment.sync.config'].sudo()
            SyncLog = self.env['nextcloud.sync.log'].sudo()
            ...
            primary_config = configs[:1] if configs else self.env['nextcloud.config']
    ...
    def get_drive_quota(self, config_id=False):
        """Fetch Nextcloud storage quota via WebDAV PROPFIND (quota-available-bytes)."""
        try:
            if config_id:
                cid = int(config_id)
                config = self.env['nextcloud.config'].sudo().browse(cid)
            else:
                config = self.env['nextcloud.config'].sudo().search([], limit=1)
            ...
            # For Basic Auth configs (Nextcloud), _get_access_token() returns None — that is
            # expected and correct. get_drive_quota() uses Basic Auth internally via _webdav_auth(),
            # so we do NOT need an OAuth token at all. Skip the token check entirely.
            quota_data = sync.get_drive_quota(config)
            return quota_data
        except Exception as e:
    ...
    def get_largest_files(self, limit=10, config_id=False):
        ...
        try:
            ...
            files = self.env['nextcloud.file'].sudo().search(
        ...
        try:
            ...
            for f in shared_records:
                ...
                result.append({
                    ...
                    'drive_url': f.nextcloud_url or '',
                    'nextcloud_file_id': f.nextcloud_file_id or '',
                    'nextcloud_url': f.nextcloud_url or '',
    ...
    def load_dashboard_layout(self):
        ...
        layout = self.env['nextcloud.dashboard.layout'].sudo().search(
    ...
    def save_dashboard_layout(self, layout_json):
        ...
        Layout = self.env['nextcloud.dashboard.layout'].sudo()
