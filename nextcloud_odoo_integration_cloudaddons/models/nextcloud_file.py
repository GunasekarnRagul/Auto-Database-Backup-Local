...
class NextcloudFile(models.Model):
    ...
    md5_checksum = fields.Char('MD5 Checksum', index=True, help="Nextcloud MD5 hash for exact content matching")
    ...
    nextcloud_file_id = fields.Char('Nextcloud File ID')
    nextcloud_url = fields.Char('Nextcloud URL')
    ...
    def _resolve_parent_nextcloud_id(self, parent_folder_id=False, root_folder_id=False):
        """Resolve the Nextcloud parent folder ID from Odoo records."""
        if parent_folder_id:
    ...
    def action_archive_recursive(self):
        """Archive records locally only. No changes made to Nextcloud."""
        for record in self:
    ...
    def action_unarchive(self):
        """Restore archived records locally. No changes made to Nextcloud."""
    ...
    def delete_on_drive_and_unlink(self, record_ids):
        """Move records to Nextcloud Trash and then unlink them from Odoo.
    ...
    def rename_on_drive_by_id(self, record_id, new_name, old_name=None):
        """Rename a file/folder on Nextcloud. 
    ...
    def action_upload_from_explorer(self, file_name, file_data, mime_type,
        ...
        The actual upload to Nextcloud happens when the user clicks Sync.
        """
    ...
    def action_get_share_info(self):
        ...
        if not self.nextcloud_file_id:
            return {'error': 'File not yet synced to Nextcloud.'}
        sync = self.env['nextcloud.sync'].sudo()
        try:
            perms = sync.get_file_permissions(self.nextcloud_file_id, self.drive_config_id)
            links = sync.get_shared_links(self.nextcloud_file_id, self.drive_config_id)
            general = 'restricted'
            if links:
                general = 'anyone'
            return {
        ...
        try:
            ...
            if isinstance(result, dict):
                if result.get('ok'):
                    ...
                    return response
                return {'error': result.get('error', 'Nextcloud could not add the member.')}
            # Legacy: plain True/False
            if result:
                self.sudo().write({'shared_people_count': self.shared_people_count + 1})
                return {'success': True}
            return {'error': 'Nextcloud could not add the member. Check the email and try again.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_update_permission(self, permission_id, role):
        ...
        try:
            ...
            if ok:
                return {'success': True}
            return {'error': 'Could not update permission on Nextcloud.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_remove_permission(self, permission_id):
        ...
        try:
            ...
            if ok:
                return {'success': True}
            return {'error': 'Could not remove permission on Nextcloud.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def action_set_general_access(self, access_type, role='reader', block_download=None, password=None, expirationDate=None):
        ...
        try:
            ...
            if result:
                self.sudo().write({'permission_type': access_type, 'anyone_role': role})
                return {'success': True}
            return {'error': 'Could not update access settings on Nextcloud.'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def get_duplicate_groups(self, config_id=None):
        ...
        Allows filtering by a specific Nextcloud config.
        """
