...
class AttachmentSyncConfig(models.Model):
    ...
    nextcloud_id = fields.Many2one('nextcloud.config', string='Nextcloud', required=True, ondelete='cascade')
    ...
    existing_module_ids = fields.Many2many('nextcloud.model.config', 
    ...
    def _compute_folder_path(self):
        for wizard in self:
            path_parts = []
            if wizard.nextcloud_id:
                path_parts.append(wizard.nextcloud_id.name)
            if wizard.nextcloud_folder_id:
                # Build ancestor chain from root → direct parent first, then append selected folder
                ancestors = []
                parent = wizard.nextcloud_folder_id.parent_folder_id
                while parent:
                    ancestors.insert(0, parent.name)
                    parent = parent.parent_folder_id
                path_parts.extend(ancestors)
                path_parts.append(wizard.nextcloud_folder_id.name)
    ...
    def _get_target_attachments(self, extra_domain=None):
        ...
        #    extra_domain filters out the chatter copy (e.g. nextcloud_file_id = False).
    ...
    @api.depends('model_name', 'sync_count', 'file_type', 'storage_mode', 'nextcloud_id', 'nextcloud_folder_id')
        ...
        for record in self:
            if record.model_name:
                ...
                all_attachments = all_attachments.filtered(
                    lambda a: a.nextcloud_file_id or (a.datas or a.raw) and a.type != 'url'
                )
    ...
    def _compute_sync_statistics(self):
        ...
        for record in self:
            if record.model_name:
                ...
                # Also exclude url-type records with no binary data and no nextcloud_file_id — these
                ...
                synced = all_attachments.filtered(lambda a: a.nextcloud_file_id)
    ...
    def _create_attachment_categories(self):
        """Create folder structure for attachment categorization in Nextcloud."""
        self.ensure_one()
        ...
        # In a real implementation, this would create subfolders in Nextcloud
        # For now, we log the structure that would be created
        folder_structure = f"""
        📂 {self.nextcloud_folder_id.name}
    ...
    def action_manual_drive_sync(self):
        ...
        if not self.nextcloud_id:
            raise UserError(
                "⚠️ No Nextcloud selected. Please choose a Nextcloud account "
                "in Step 2 before syncing."
            )

        if not self.nextcloud_folder_id:
            raise UserError(
            ...
            return {
                ...
                'params': {
                    ...
                    'message': (
                        ...
                        'No files will be uploaded to Nextcloud. '
        ...
        attachments = self._get_target_attachments([('nextcloud_file_id', '=', False)])
        ...
        if not attachments:
            ...
            return {
                ...
                'params': {
                    ...
                    'message': (
                        f'All {model_label} files are already synced to Nextcloud.\n'
                        f'Storage Mode: {mode_label} | Drive: {self.nextcloud_id.name}'
                    ),
                    'type': 'info',
                    'sticky': False,
                }
        ...
        for attachment in attachments:
            ...
            try:
                ...
                failed_count += 1
                # Log the failure to Activity Log so it's visible to the user
                import logging
                _log = logging.getLogger(__name__)
                _log.error("Manual sync failed for attachment '%s': %s", attachment.name, e)
                try:
                    self.env['nextcloud.sync.log'].log_operation(
                        ...
                        state='fail',
                        sync_type='manual',
                        root_folder_name=self.nextcloud_folder_id.name if self.nextcloud_folder_id else '',
                        folder_path=self.model_name or '',
                        error_message=str(e),
                    )
                except Exception:
                    pass
        ...
        if synced_count > 0:
            message = (
                ...
                f"{synced_count} {model_label} file(s) synced to Nextcloud.\n"
                f"Storage Mode: {mode_label} | Drive: {self.nextcloud_id.name}"
            )
            if failed_count:
    ...
    def action_setup_auto_sync(self):
        """Setup auto-sync: push all unsynced historical attachments to Nextcloud.
        ...
        if attachments:
            for attachment in attachments:
                try:
                    ...
                    import logging
                    _log = logging.getLogger(__name__)
                    _log.error("Auto-setup sync failed for attachment '%s': %s", attachment.name, e)
                    try:
                        self.env['nextcloud.sync.log'].log_operation(
                            config=self.nextcloud_id,
                            file_name=attachment.name,
        ...
        model_label = self.module_config_id.model_label or self.model_name
        message = f'Auto-sync setup complete. {synced_count} historical file(s) moved to Nextcloud for {model_label}.'
        if failed_count:
    ...
    def action_apply_sync(self):
        """Push all unsynced historical attachments to Nextcloud.
