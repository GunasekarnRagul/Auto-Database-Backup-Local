...
export class SyncLogTerminal extends Component {
        ...
        try {
            const logs = await this.orm.searchRead(
                ...
                [
                    ...
                    "nextcloud_file_id", "file_size", "display_name", "folder_path"
                ],
            ...
            const successCount = await this.orm.searchCount("nextcloud.sync.log", sDomain);

            // 2. Fail Count for current filters
            const fDomain = statsDomain.filter(d => d[0] !== 'state');
            fDomain.push(['state', '=', 'fail']);
            const failCount = await this.orm.searchCount("nextcloud.sync.log", fDomain);
    ...
    async onClearLogs() {
        ...
        try {
            const allIds = await this.orm.search("nextcloud.sync.log", []);
            if (allIds.length) {
                await this.orm.unlink("nextcloud.sync.log", allIds);
            }
            await this.loadLogs();
        } catch (e) {
...
SyncLogTerminal.template = "nextcloud_odoo_integration.SyncLogTerminal";

registry.category("actions").add("nextcloud_odoo_integration.sync_log_terminal", SyncLogTerminal);
