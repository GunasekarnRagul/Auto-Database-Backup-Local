...
export class FileExplorer extends Component {
    ...
    getFileImageUrl(file, isPreview = false) {
        ...
        const mime = file.mime_type || '';
        const basePath = '/nextcloud_odoo_integration_cloudaddons/static/description/';
    ...
    onFileClick(ev, file) {
        ...
        if (file.file_type === 'folder') {
            ...
            // Nextcloud's previewer handles virtually all formats natively (PDF, Office,
        ...
        if (file.nextcloud_file_id) {
            try {
                ...
                if (url) {
                    ...
                    this.state.error = "Nextcloud could not generate a preview for this file type.";
                }
            } catch (e) {
    ...
    async loadShareInfo() {
        ...
        try {
            ...
            if (result.error) {
                ...
                const now = new Date();
                this.state.lastSyncTime = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                // Start auto-polling so Nextcloud-side changes reflect here every 30s
                this._startAutoSync();
            }
        } catch (e) {
