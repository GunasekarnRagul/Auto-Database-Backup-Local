...
/**
 * Nextcloud – Customizable Widget Dashboard (v3 — No External Dependencies)
 *
 * Architecture:
...
export class NextcloudDashboard extends Component {
    setup() {
    ...
    // so our own .nextcloud-dash can scroll internally.
    _fixParentScroll() {
        let el = this.__owl__?.bdom?.el || document.querySelector(".nextcloud-dash");
        if (!el) return;
    ...
    async _loadLayout() {
        try {
            const saved = await this.orm.call("nextcloud.dashboard", "load_dashboard_layout", []);
            if (saved) {
    ...
    async _saveLayout() {
        try {
            await this.orm.call("nextcloud.dashboard", "save_dashboard_layout",
    ...
    async _fetchCoreData() {
        ...
        try {
            const d = await this.orm.call("nextcloud.dashboard", "get_kpi_data", []);
    ...
    async _fetchSyncTrendData() {
        try {
            const d = await this.orm.call("nextcloud.dashboard", "get_sync_trend_data", [], { 
    ...
    async _loadLazyWidgets() {
        const s = this.state;
        const tasks = [
            this.orm.call("nextcloud.dashboard", "get_activity_logs", [], { period: s.logFilter })
                .then(d => { s.recent_logs = d; }),
            this.orm.call("nextcloud.dashboard", "get_storage_stats", [])
                .then(d => { Object.assign(s.kpis, d); }),
            this.orm.call("nextcloud.dashboard", "get_duplicate_summary", [])
                .then(d => { s.duplicate_count = d.duplicate_count; }),
            this.orm.call("nextcloud.dashboard", "get_model_breakdown", [])
                .then(d => { s.model_breakdown = d; }),
            this.orm.call("nextcloud.dashboard", "get_error_summary", [])
                .then(d => { s.error_summary = d; }),
            this.orm.call("nextcloud.dashboard", "get_sync_trend_data", [], { start_date: s.globalTrendStart, end_date: s.globalTrendEnd })
                .then(d => { s.trendData = d; }),
            this.orm.call("nextcloud.dashboard", "get_files_by_model", [])
                .then(d => { s.files_by_model = d; }),
            this.orm.call("nextcloud.dashboard", "get_top_uploaders", [])
                .then(d => { s.top_uploaders = d; }),
            this.orm.call("nextcloud.dashboard", "get_top_file_types", [], { config_id: s.tft_drive_id || false })
                .then(d => { if (!s.top_file_types_data) s.top_file_types_data = {}; s.top_file_types_data[s.tft_drive_id] = d; s.top_file_types = d; }),
            this.orm.call("nextcloud.dashboard", "get_largest_files", [])
                .then(d => { s.largest_files = d; }),
            this._widgetFetchers.recent_files(),
            this._widgetFetchers.active_shares(),
            this.orm.call("nextcloud.dashboard", "get_storage_by_model", [])
                .then(d => { s.storage_by_model = d; }),
            this.orm.call("nextcloud.dashboard", "get_orphan_attachments", [])
        ...
        if (s.health && s.health.fleet) {
            for (const drive of s.health.fleet) {
                ...
                this.orm.call("nextcloud.dashboard", "get_drive_quota", [], { config_id: drive.id })
    ...
    get _widgetFetchers() {
        const s = this.state;
        return {
            kpi_total_files:  async () => { const d = await this.orm.call("nextcloud.dashboard", "get_kpi_data", []); Object.assign(s.kpis, d.kpis); s.health = d.health; },
            kpi_total_size:   async () => { const d = await this.orm.call("nextcloud.dashboard", "get_storage_stats", []); Object.assign(s.kpis, d); },
            kpi_files_month:  async () => { const d = await this.orm.call("nextcloud.dashboard", "get_kpi_data", []); Object.assign(s.kpis, d.kpis); },
            kpi_pending:      async () => { const d = await this.orm.call("nextcloud.dashboard", "get_kpi_data", []); Object.assign(s.kpis, d.kpis); },
            ...
            top_file_types:   async () => { 
                if (s.tft_drive_id) {
                    const d = await this.orm.call("nextcloud.dashboard", "get_top_file_types", [], { config_id: s.tft_drive_id });
                    if (!s.top_file_types_data) s.top_file_types_data = {};
                    s.top_file_types_data[s.tft_drive_id] = d;
                }
            },
            largest_files:    async () => { const d = await this.orm.call("nextcloud.dashboard", "get_largest_files", []); s.largest_files = d; },
            sync_trend:       async () => { const d = await this.orm.call("nextcloud.dashboard", "get_sync_trend_data", [], { start_date: s.globalTrendStart, end_date: s.globalTrendEnd }); s.trendData = d; },
            failed_syncs:     async () => { const d = await this.orm.call("nextcloud.dashboard", "get_error_summary", []); s.error_summary = d; },
            duplicates:       async () => { const d = await this.orm.call("nextcloud.dashboard", "get_duplicate_summary", []); s.duplicate_count = d.duplicate_count; },
            activity_log:     async () => { const d = await this.orm.call("nextcloud.dashboard", "get_activity_logs", [], { period: s.logFilter }); s.recent_logs = d; },
            recent_files: async () => {
                ...
                const records = await this.orm.searchRead(
                    ...
                    ['id', 'name', 'file_size', 'mime_type', 'nextcloud_url', 'nextcloud_file_id', 'sync_state', 'write_date']
                );
                ...
                s.recent_files = records.map(f => ({
                    ...
                    drive_url: f.nextcloud_url || '',
                    nextcloud_file_id: f.nextcloud_file_id || '',
                    date: f.write_date ? f.write_date.replace('T', ' ').slice(0, 16) : '',
                    sync_state: f.sync_state || '',
                }));
            },
            active_shares:    async () => { 
                const d = await this.orm.call("nextcloud.dashboard", "get_active_shares", [], { 
                    config_id: s.asl_drive_id || false 
                }); 
                s.active_shares = d; 
            },
            top_uploaders:    async () => { const d = await this.orm.call("nextcloud.dashboard", "get_top_uploaders", []); s.top_uploaders = d; },
            model_breakdown:  async () => { const d = await this.orm.call("nextcloud.dashboard", "get_model_breakdown", []); s.model_breakdown = d; },
        };
    }
        ...
        return {
            ...
            storage_meter: el => {
                ...
                if (!q || (!q.available && !q.error)) {
                    ...
                    this.orm.call("nextcloud.dashboard", "get_drive_quota", [], { config_id: s.sm_drive_id })
            ...
            top_file_types: el => {
                ...
                if (!d) {
                    ...
                    this.orm.call("nextcloud.dashboard", "get_top_file_types", [], { config_id: s.tft_drive_id })
    ...
    async _filterLogs(period) {
        ...
        try {
            const logs = await this.orm.call("nextcloud.dashboard", "get_activity_logs", [], { period });
    ...
    _openSyncLogs() {
        this.action.doAction({ type: "ir.actions.act_window", res_model: "nextcloud.sync.log",
    ...
    _openDriveSettings(id) {
        this.action.doAction({ type: "ir.actions.act_window", res_model: "nextcloud.config",
    ...
    openFileExplorer() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "nextcloud_odoo_integration.file_explorer", name: "File Explorer" });
    }

    openActivityLogs() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "nextcloud_odoo_integration.sync_log_terminal", name: "Activity Log Terminal" });
    }
...
NextcloudDashboard.template = "nextcloud_odoo_integration.Dashboard";
registry.category("actions").add("nextcloud_odoo_integration.dashboard", NextcloudDashboard);
