...
class WhatsAppMessage(models.Model):
    ...
    def _send_to_whatsapp(self):
        ...
        if self.recipient_type == 'single':
            ...
            for contact in self.recipient_contacts:
                mobile = getattr(contact, 'mobile', False)
                phone = getattr(contact, 'phone', False)
                if mobile:
                    numbers_to_send.append(mobile.strip())
                elif phone:
                    numbers_to_send.append(phone.strip())
    ...
    def action_send_message(self):
        ...
        if self.recipient_type == 'single':
            ...
            raw_multi = self.recipient_multi or ""
            numbers_to_send = [x.strip() for x in raw_multi.split(',') if x.strip()]
