...
class AttachmentSyncConfig(models.Model):
    ...
    model_attachment_count = fields.Integer('Total Attachments', compute='_compute_sync_statistics')
    ...
    def _compute_sync_statistics(self):
        """Compute detailed sync statistics for the dashboard/tree view.
        Uses optimized SQL CTE queries to get deduplicated direct and chatter attachments,
        avoiding heavy model instantiation and subqueries.
        """
        for record in self:
            if record.model_name:
                ...
                self.env.cr.execute("""
                    WITH model_msgs AS (
                        SELECT id, res_id FROM mail_message 
                        WHERE model = %s AND res_id IS NOT NULL AND res_id != 0
                    ),
                    mail_atts AS (
                        SELECT 
                            a.id, 
                            m.res_id as target_res_id, 
                            a.name, 
                            a.one_drive_file_id, 
                            a.type, 
                            (a.db_datas IS NOT NULL OR a.store_fname IS NOT NULL OR a.file_size > 0) as has_data,
                            a.mimetype
                        FROM ir_attachment a
                        JOIN model_msgs m ON a.res_model = 'mail.message' AND a.res_id = m.id
                        WHERE (a.res_field IS NULL OR a.res_field = '')
                    ),
                    direct_atts AS (
                        SELECT 
                            id, 
                            res_id as target_res_id, 
                            name, 
                            one_drive_file_id, 
                            type, 
                            (db_datas IS NOT NULL OR store_fname IS NOT NULL OR file_size > 0) as has_data,
                            mimetype
                        FROM ir_attachment
                        WHERE res_model = %s AND res_id IS NOT NULL AND res_id != 0
                          AND (res_field IS NULL OR res_field = '')
                    )
                    SELECT id, one_drive_file_id, type, has_data, mimetype, name
                    FROM mail_atts
                    ...
                    FROM direct_atts d
                    WHERE NOT EXISTS (
                        SELECT 1 FROM mail_atts m 
                        WHERE m.target_res_id = d.target_res_id AND m.name = d.name
                    )
                """, (m_name, m_name))
                ...
                valid_rows = []
                for r in rows:
                    is_valid = bool(r['one_drive_file_id']) or (bool(r['has_data']) and r['type'] != 'url')
                    if not is_valid:
                        continue
                    ...
                    if record.file_type != 'all':
                        ...
                        if record.file_type == 'pdf':
                            matched = 'pdf' in mimetype or name.endswith('.pdf')
                        elif record.file_type == 'doc':
                            matched = 'word' in mimetype or 'document' in mimetype or name.endswith(('.doc', '.docx'))
                        elif record.file_type == 'xls':
                            matched = 'excel' in mimetype or 'spreadsheet' in mimetype or name.endswith(('.xls', '.xlsx'))
                        elif record.file_type == 'ppt':
                            matched = 'powerpoint' in mimetype or 'presentation' in mimetype or name.endswith(('.ppt', '.pptx'))
                        elif record.file_type == 'img':
                            matched = mimetype.startswith('image/') or name.endswith(('.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg'))
                        if not matched:
                            continue
                    
                    valid_rows.append(r)

                synced = [r for r in valid_rows if r['one_drive_file_id']]
                unsynced = [r for r in valid_rows if not r['one_drive_file_id']]
                dual = [r for r in synced if r['type'] != 'url']
                drive_only = [r for r in synced if r['type'] == 'url']
                ...
                record.model_attachment_count = total
            else:
