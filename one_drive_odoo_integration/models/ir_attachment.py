...
class IrAttachment(models.Model):
    ...
    def _compute_one_drive_id(self):
        """Get the drive associated with this attachment. Fallback to model config if not synced."""
        for attachment in self:
            if attachment.one_drive_file_id:
                file_record = self.env['one.drive.file'].sudo().search([
                    ('one_drive_file_id', '=', attachment.one_drive_file_id)
                ], limit=1)
                attachment.one_drive_id = file_record.drive_config_id.id if file_record else False
            else:
                ...
                attachment.one_drive_id = config.one_drive_id.id if config else False
    ...
    def _compute_model_display(self):
        """Get friendly display name for the model using one_drive.model.config."""
        for attachment in self:
            ...
            config = self.env['one_drive.model.config'].sudo().search([
                ('res_model', '=', res_model)
            ], limit=1)
    ...
    def _compute_google_folder_path(self):
        ...
        for attachment in self:
            path_parts = []
            if attachment.one_drive_id:
                path_parts.append(attachment.one_drive_id.name)
    ...
    def get_model_config(self):
        ...
        if not self.res_model:
            return False
        return self.env['one_drive.model.config'].sudo().search([
    ...
    def create(self, vals_list):
        ...
        for attachment in attachments:
            if self.env.context.get('skip_one_drive_sync'):
                continue
            ...
            # Setting one_drive_file_id on the mail.message record makes the chatter
            ...
            if res_model == 'mail.message' and res_id:
                ...
                if not attachment.datas and not attachment.raw:
                    ...
                    if attachment.type == 'url' and attachment.url and ('onedrive.live.com' in attachment.url or 'sharepoint.com' in attachment.url):
                        ...
                        if res_id_param:
                            attachment.with_context(skip_one_drive_sync=True).write({
                                'one_drive_file_id': res_id_param
                            })
                    ...
                    if direct:
                        drive_url = direct.url or \
                            f"https://onedrive.live.com/redir?resid={direct.one_drive_file_id}"
                        attachment.with_context(skip_one_drive_sync=True).write({
                            'one_drive_file_id': direct.one_drive_file_id,
                            'type': 'url',
                            'url': drive_url,
                        })
    ...
    def _inject_chatter_drive_button(self, drive_url, file_name=None, unlink_card=True):
        ...
        btn_html = (
            ...
            "<b><i class='fa fa-windows'></i> OneDrive Documents:</b>"
        ...
        for msg in linked_messages:
            ...
            if self.res_model == 'mail.message':
                self.with_context(skip_one_drive_sync=True).write({'res_model': 'one.drive.discard'})
    ...
    def _auto_sync_to_drive(self, config):
        ...
        parent_id = config.google_folder_id.one_drive_file_id
        if not parent_id and config.one_drive_id.root_ids:
             root = config.one_drive_id.root_ids.filtered(lambda r: r.active)[:1]
        ...
        def get_or_create_local_folder(f_name, parent_local, config_drive_id, one_drive_id, one_drive_url, folder_res_model=False, folder_res_id=False):
            ...
            local_folder = self.env['one.drive.file'].sudo().search(search_domain, limit=1)
            if not local_folder:
                ...
                try:
                    with self.env.cr.savepoint():
                        local_folder = self.env['one.drive.file'].sudo().create({
                            'name': f_name,
                            'file_type': 'folder',
                            'drive_config_id': config_drive_id,
                            'parent_folder_id': parent_local,
                            'one_drive_file_id': one_drive_id,
        ...
        try:
            if model_root and record_folder and current_drive_parent_id:
                ...
                root_res = sync_service.find_or_create_folder(model_root, current_drive_parent_id, config.one_drive_id)
                if root_res:
                    ...
                    for part in path_parts:
                        if not part:
                            continue
                        part_res = sync_service.find_or_create_folder(part, last_res['one_drive_file_id'], config.one_drive_id)
                        if part_res:
                    ...
                    parent_id = last_res['one_drive_file_id']
            ...
            if result:
                ...
                att_vals = {
                    ...
                    'one_drive_id': config.one_drive_id.id,
                ...
                # have no one_drive_file_id because the Step 1 inheritance search ran BEFORE
                # the direct attachment had a one_drive_file_id. Fix all such ghosts now.
                if storage_mode == 'drive':
                    res_model_target = (
                    ...
                    if res_model_target and res_id_target \
                        ...
                        if messages:
                            ...
                            synced_url = result['one_drive_url']
                            synced_gid = result['one_drive_file_id']
                            for ghost in ghost_atts:
                                ...
                                if self._normalize_att_name(ghost.name) != \
                                    ...
                                    continue
                                ghost.with_context(skip_one_drive_sync=True).write({
                                    'one_drive_file_id': synced_gid,
                                    'type': 'url',
                                    'url': synced_url,
                                    'datas': False,
                                    'db_datas': False,
                ...
                existing_drive_file = self.env['one.drive.file'].sudo().search([
                    ('one_drive_file_id', '=', result['one_drive_file_id']),
                    ('drive_config_id', '=', config.one_drive_id.id),
                ], limit=1)
                ...
                self.env['one.drive.sync'].sudo()._notify_folder_sync(current_local_parent_id)
                ...
                if config.storage_mode == 'drive':
                    details = (
                        ...
                        f"Drive URL: {result['one_drive_url']}"
                    )
                else:
                    details = (
                        f"Synced At: {ts}\n"
    ...
    def action_sync_to_drive(self):
        ...
        if config:
            ...
            if not self.one_drive_id:
                active_drives = self.env['one.drive.config'].sudo().search([('active', '=', True)], limit=1)
                if active_drives:
            ...
            try:
                ...
                parent_one_drive_id = False
                if self.google_folder_id and self.google_folder_id.one_drive_file_id:
                    parent_one_drive_id = self.google_folder_id.one_drive_file_id
                elif self.one_drive_id.root_ids:
                    root = self.one_drive_id.root_ids.filtered(lambda r: r.active)[:1]
                    parent_one_drive_id = root.root_id if root else False
                ...
                result = sync.upload_file_to_drive(
                    ...
                    self.mimetype or 'application/octet-stream',
                    self.one_drive_id,
                    parent_one_drive_id=parent_one_drive_id
                )

                if result:
                    self.with_context(skip_one_drive_sync=True).write({
                        'one_drive_file_id': result['one_drive_file_id'],
                        'one_drive_id': self.one_drive_id.id,
                        'google_folder_id': self.google_folder_id.id,
                    })
                    
                    # Log as manual upload
                    self.env['one.drive.sync.log'].log_operation(
                        config=self.one_drive_id,
                        file_name=self.name,
                        operation='upload',
                        state='success',
                        sync_type='manual',
    ...
    def _compute_raw(self):
        ...
        for attach in self:
            if not attach.raw and attach.one_drive_file_id:
                try:
                    config = attach.one_drive_id
                    if not config:
                        res_model = attach.res_model
                        res_id = attach.res_id
                        if res_model == 'mail.message' and res_id:
                            res_model, _ = attach._resolve_mail_message_parent(res_id)
                        sync_config = attach.env['attachment.sync.config'].sudo().get_config_for_model(res_model)
                        config = sync_config.one_drive_id if sync_config else False
                    
                    if config:
                        access_token = attach.env['one.drive.sync'].sudo().with_context(skip_one_drive_sync=True)._get_access_token(config)
                        if access_token:
                            import requests
                            headers = {'Authorization': f'Bearer {access_token}'}
                            response = requests.get(
                                f"https://graph.microsoft.com/v1.0/me/drive/items/{attach.one_drive_file_id}/content",
                                headers=headers,
                                allow_redirects=True,
                                timeout=30
                            )
                            if response.status_code == 200:
                                attach.raw = response.content
                                continue
                except Exception as e:
                    _logger.error("Failed to download OneDrive attachment content on-the-fly: %s", str(e))
                attach.raw = b''

    def _to_http_stream(self):
        self.ensure_one()
        if self.one_drive_file_id:
