...
class GoogleDriveRootFolder(models.Model):
    ...
    _description = 'OneDrive Root Folder'

    name = fields.Char('Folder Name', required=True)
    root_id = fields.Char('OneDrive Folder ID', readonly=True)
    ...
    def unlink(self):
        ...
        self.env['one.drive.file'].sudo().search([
...
class GoogleDriveConfig(models.Model):
    ...
    def _compute_redirect_uri(self):
        ...
        for config in self:
            ...
            config.redirect_uri = f"{host_url}/one_drive/authentication"
    ...
    def compute_redirect_from_request(self):
        ...
        for config in self:
            ...
            redirect = f"{host_root}/one_drive/authentication"
    ...
    def action_trigger_sync(self, root_folder_id=None, drive_config_id=None):
        ...
        """
        # Step 1: Push any pending folders/files to OneDrive first
        # Scope push to drive if provided
        self.env['one.drive.file'].sudo().with_context(sync_type='manual').sync_pending_to_drive(drive_config_id=drive_config_id)
    ...
    def action_auto_sync_drive(self, drive_config_id=None):
        """One-way auto-sync: push pending items for a specific drive to OneDrive.

        This is the method called by auto-sync mode in the file explorer. It is
        intentionally ONE-WAY (Odoo → OneDrive only) to keep auto-sync fast
        and safe while the user is actively working.

        It does NOT pull changes from OneDrive. Use action_trigger_sync
        (Manual Sync) for a full bi-directional sync.

        Args:
            drive_config_id (int|None): ID of the one.drive.config to sync.
