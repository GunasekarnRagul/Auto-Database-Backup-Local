...
class DuplicatePrunerWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        groups = self.env['one.drive.file'].get_duplicate_groups()
        res.update(self._prepare_wizard_values(groups))
        return res

    @api.onchange('drive_config_id')
    def _onchange_drive_config_id(self):
        groups = self.env['one.drive.file'].get_duplicate_groups(config_id=self.drive_config_id.id)
...
class DuplicatePrunerWizardLine(models.TransientModel):
    ...
    file_id = fields.Many2one('one.drive.file', string="File")
    ...
    drive_config_id = fields.Many2one('one.drive.config')
    root_folder_id = fields.Many2one('one.drive.root.folder')
    parent_folder_id = fields.Many2one('one.drive.file')

    def action_view_in_explorer(self):
        self.ensure_one()
        url = "/web#action=one_drive_odoo_integration.action_one_drive_file_explorer"
