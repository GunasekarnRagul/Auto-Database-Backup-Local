...
/**
 * OneDrive – Customizable Widget Dashboard (v3 — No External Dependencies)
 *
 * Architecture:
...
export class GoogleDriveDashboard extends Component {
    ...
    openFileExplorer() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "one_drive_odoo_integration.file_explorer", name: "File Explorer" });
    }

    openActivityLogs() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "one_drive_odoo_integration.sync_log_terminal", name: "Activity Log Terminal" });
    }
...
GoogleDriveDashboard.template = "one_drive_odoo_integration.Dashboard";
registry.category("actions").add("one_drive_odoo_integration.dashboard", GoogleDriveDashboard);
