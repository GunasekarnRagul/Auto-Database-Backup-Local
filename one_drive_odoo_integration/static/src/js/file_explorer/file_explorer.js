...
export class FileExplorer extends Component {
    ...
    getFileImageUrl(file, isPreview = false) {
        ...
        const mime = file.mime_type || '';
        const basePath = '/one_drive_odoo_integration/static/description/';
    ...
    onFileClick(ev, file) {
        ...
        if (file.file_type === 'folder') {
            ...
            // OneDrive's previewer handles virtually all formats natively (PDF, Office,
            ...
            if (file.one_drive_file_id && !canSkipPreview) {
                this.dialogService.add(FilePreviewDialog, {
                    file: file,
                    onDownload: () => {
                        window.open(`/one_drive/download/${file.id}`, '_blank');
                    },
                    onOpenDetails: () => {
    ...
    async onPaste() {
        ...
        if (this.state.activeDriveId !== this.state.clipboard.sourceDriveId) {
            this.notificationService.add("Other drive not pasted. Moving items across different OneDrives is not supported.", { 
                type: "danger",
                sticky: true 
            });
            return;
    ...
    onDownloadSelected() {
        ...
        if (hasFolder || isMulti) {
            ...
            const downloadUrl = `/one_drive/download_zip?file_ids=${ids}`;
        ...
        const file = files[0];
        const downloadUrl = `/one_drive/download/${file.id}`;
        const a = document.createElement('a');
        a.style.display = 'none';
        a.href = downloadUrl;
    ...
    async onDeleteSelected() {
        ...
        try {
            ...
            this.notificationService.add(
                `Moved to Odoo Trash: ${nameList}. These items will remain active on OneDrive until permanently deleted from the Trash tab.`,
                { type: "info" }
            );
            await this.checkPendingChanges();
        } catch (e) {
        ...
        try {
            ...
            if (file.one_drive_file_id) {
                try {
                    ...
                    this.notificationService.add(`Renamed to "${newName}" on OneDrive`, { type: "success" });
                } catch (e) {
                    this.notificationService.add(`Failed to rename "${newName}" on OneDrive`, { type: "warning" });
                }
            } else {
    ...
    /**
     * Trigger a one-way auto-sync (Odoo → OneDrive) for a specific drive.
     *
...
FileExplorer.template = "one_drive_odoo_integration.FileExplorer";

registry.category("actions").add("one_drive_odoo_integration.file_explorer", FileExplorer);

class FilePreviewDialog extends Component {
    static template = "one_drive_odoo_integration.FilePreviewDialog";
...
export class ShareDriveLinkDialog extends Component {
    static template = "one_drive_odoo_integration.ShareDriveLinkDialog";
    static components = { Dialog };
    static props = {
        files: Array,
        close: Function,
    ...
    get shareLink() {
        ...
        return file.one_drive_url || `https://onedrive.live.com/redir?resid=${file.one_drive_file_id}`;
    }
