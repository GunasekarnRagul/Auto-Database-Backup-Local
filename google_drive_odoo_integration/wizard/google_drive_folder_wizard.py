...
class GoogleDriveFolderWizard(models.TransientModel):
    _name = 'google.drive.folder.wizard'
    _description = 'Google Drive Folder Bulk Add Wizard'

    config_id = fields.Many2one('google.drive.config', string='Drive Configuration', required=True)
    folder_ids_raw = fields.Text('Folder IDs', help='Paste Google Folder IDs here, one per line.')
    
    line_ids = fields.One2many('google.drive.folder.wizard.line', 'wizard_id', string='Available Folders')

    def action_fetch_folders(self):
        """Fetch top-level folders from the connected Google Drive."""
        self.ensure_one()
        # Clear existing lines
        self.line_ids.unlink()
        
        sync_obj = self.env['google.drive.sync'].sudo()
        ...
        if not access_token:
            return {
                ...
                'params': {
                    ...
                    'message': _('Could not connect to Google Drive. Please check your credentials.'),
                    'sticky': False,
                }
            }
    ...
    def action_apply(self):
        self.ensure_one()
        root_obj = self.env['google.drive.root.folder']
...
class GoogleDriveFolderWizardLine(models.TransientModel):
    _name = 'google.drive.folder.wizard.line'
    _description = 'Google Drive Folder Bulk Add Wizard Line'

    wizard_id = fields.Many2one('google.drive.folder.wizard', string='Wizard')
