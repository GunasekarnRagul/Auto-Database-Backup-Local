...
class DuplicatePrunerWizard(models.TransientModel):
    ...
    drive_config_id = fields.Many2one('google.drive.config', string='Google Drive', 
    ...
    def _onchange_drive_config_id(self):
        groups = self.env['google.drive.file'].get_duplicate_groups(config_id=self.drive_config_id.id)
...
class DuplicatePrunerWizardLine(models.TransientModel):
    ...
    file_id = fields.Many2one('google.drive.file', string="File")
    ...
    drive_config_id = fields.Many2one('google.drive.config')
    root_folder_id = fields.Many2one('google.drive.root.folder')
    parent_folder_id = fields.Many2one('google.drive.file')

    def action_view_in_explorer(self):
        self.ensure_one()
        url = "/web#action=google_drive_odoo_integration.action_google_drive_file_explorer"
