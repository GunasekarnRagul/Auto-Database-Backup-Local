...
/**
 * Google Drive – Customizable Widget Dashboard (v3 — No External Dependencies)
 *
 * Architecture:
...
export class GoogleDriveDashboard extends Component {
    ...
    // so our own .gdrive-dash can scroll internally.
    _fixParentScroll() {
        let el = this.__owl__?.bdom?.el || document.querySelector(".gdrive-dash");
        if (!el) return;
    ...
    async _loadLayout() {
        try {
            const saved = await this.orm.call("google.drive.dashboard", "load_dashboard_layout", []);
            if (saved) {
    ...
    async _saveLayout() {
        try {
            await this.orm.call("google.drive.dashboard", "save_dashboard_layout",
    ...
    async _fetchSyncTrendData() {
        try {
            const d = await this.orm.call("google.drive.dashboard", "get_sync_trend_data", [], { 
    ...
    async _loadLazyWidgets() {
        const s = this.state;
        const tasks = [
            this.orm.call("google.drive.dashboard", "get_activity_logs", [], { period: s.logFilter })
                .then(d => { s.recent_logs = d; }),
            this.orm.call("google.drive.dashboard", "get_storage_stats", [])
                .then(d => { Object.assign(s.kpis, d); }),
            this.orm.call("google.drive.dashboard", "get_duplicate_summary", [])
                .then(d => { s.duplicate_count = d.duplicate_count; }),
            this.orm.call("google.drive.dashboard", "get_model_breakdown", [])
                .then(d => { s.model_breakdown = d; }),
            this.orm.call("google.drive.dashboard", "get_error_summary", [])
                .then(d => { s.error_summary = d; }),
            this.orm.call("google.drive.dashboard", "get_sync_trend_data", [], { start_date: s.globalTrendStart, end_date: s.globalTrendEnd })
                .then(d => { s.trendData = d; }),
            this.orm.call("google.drive.dashboard", "get_files_by_model", [])
        ...
        return {
            ...
            storage_meter: el => {
                ...
                if (!q || (!q.available && !q.error)) {
                    ...
                    this.orm.call("google.drive.dashboard", "get_drive_quota", [], { config_id: s.sm_drive_id })
    ...
    _openSyncLogs() {
        this.action.doAction({ type: "ir.actions.act_window", res_model: "google.drive.sync.log",
    ...
    openActivityLogs() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "google_drive_odoo_integration.sync_log_terminal", name: "Activity Log Terminal" });
    }
...
GoogleDriveDashboard.template = "google_drive_odoo_integration.Dashboard";
registry.category("actions").add("google_drive_odoo_integration.dashboard", GoogleDriveDashboard);
