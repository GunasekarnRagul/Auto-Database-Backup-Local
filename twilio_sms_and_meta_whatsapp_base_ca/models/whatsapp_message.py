...
class WhatsAppMessage(models.Model):
    ...
    whatsapp_config_id = fields.Many2one(
        'whatsapp.config', string="WhatsApp Account",
        domain="[('connection_status', '=', 'connected')]",
        help="Select the WhatsApp account to send from"
    )
    ...
    def _send_to_whatsapp(self):
        ...
        # 1. Config Check
        config = self.whatsapp_config_id or self.env['whatsapp.config'].search([('connection_status', '=', 'connected')], limit=1)
        if not config:
            raise UserError("Please configure and connect a WhatsApp account in Settings first.")
    ...
    def _upload_media_to_whatsapp(self):
        ...
        # WhatsApp API accepted MIME types - map unsupported ones to compatible alternatives
        WA_MIME_OVERRIDES = {
            'text/csv': 'text/plain',
            'application/csv': 'text/plain',
        }
        
        if mime_type in WA_MIME_OVERRIDES:
            mime_type = WA_MIME_OVERRIDES[mime_type]
