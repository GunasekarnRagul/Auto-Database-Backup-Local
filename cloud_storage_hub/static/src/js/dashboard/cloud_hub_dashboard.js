...
export class CloudHubDashboard extends Component {
    setup() {
    ...
    async _loadLayout() {
        try {
            const saved = await this.orm.call("cloud.dashboard", "load_dashboard_layout", []);
            if (saved) {
    ...
    async _saveLayout() {
        try {
            await this.orm.call("cloud.dashboard", "save_dashboard_layout",
    ...
    async _fetchCoreData() {
        ...
        try {
            const d = await this.orm.call("cloud.dashboard", "get_kpi_data", []);
    ...
    async _fetchSyncTrendData() {
        try {
            const d = await this.orm.call("cloud.dashboard", "get_sync_trend_data", [], { 
    ...
    async _loadLazyWidgets() {
        const s = this.state;
        const tasks = [
            this.orm.call("cloud.dashboard", "get_activity_logs", [], { period: s.logFilter })
                .then(d => { s.recent_logs = d; }),
            this.orm.call("cloud.dashboard", "get_storage_stats", [])
                .then(d => { Object.assign(s.kpis, d); }),
            this.orm.call("cloud.dashboard", "get_duplicate_summary", [])
                .then(d => { s.duplicate_count = d.duplicate_count; }),
            this.orm.call("cloud.dashboard", "get_model_breakdown", [])
                .then(d => { s.model_breakdown = d; }),
            this.orm.call("cloud.dashboard", "get_error_summary", [])
                .then(d => { s.error_summary = d; }),
            this.orm.call("cloud.dashboard", "get_sync_trend_data", [], { start_date: s.globalTrendStart, end_date: s.globalTrendEnd })
                .then(d => { s.trendData = d; }),
            this.orm.call("cloud.dashboard", "get_files_by_model", [])
                .then(d => { s.files_by_model = d; }),
            this.orm.call("cloud.dashboard", "get_top_uploaders", [])
                .then(d => { s.top_uploaders = d; }),
            this.orm.call("cloud.dashboard", "get_top_file_types", [], { config_id: s.tft_drive_id || false })
                .then(d => { if (!s.top_file_types_data) s.top_file_types_data = {}; s.top_file_types_data[s.tft_drive_id] = d; s.top_file_types = d; }),
            this.orm.call("cloud.dashboard", "get_largest_files", [])
                .then(d => { s.largest_files = d; }),
            this._widgetFetchers.recent_files(),
            this._widgetFetchers.active_shares(),
            this.orm.call("cloud.dashboard", "get_storage_by_model", [])
                .then(d => { s.storage_by_model = d; }),
            this.orm.call("cloud.dashboard", "get_orphan_attachments", [])
        ...
        if (s.health && s.health.fleet) {
            for (const drive of s.health.fleet) {
                ...
                this.orm.call("cloud.dashboard", "get_drive_quota", [], { config_id: drive.id })
    ...
    get _widgetFetchers() {
        const s = this.state;
        return {
            kpi_total_files:  async () => { const d = await this.orm.call("cloud.dashboard", "get_kpi_data", []); Object.assign(s.kpis, d.kpis); s.health = d.health; },
            kpi_total_size:   async () => { const d = await this.orm.call("cloud.dashboard", "get_storage_stats", []); Object.assign(s.kpis, d); },
            kpi_files_month:  async () => { const d = await this.orm.call("cloud.dashboard", "get_kpi_data", []); Object.assign(s.kpis, d.kpis); },
            kpi_pending:      async () => { const d = await this.orm.call("cloud.dashboard", "get_kpi_data", []); Object.assign(s.kpis, d.kpis); },
            ...
            top_file_types:   async () => { 
                if (s.tft_drive_id) {
                    const d = await this.orm.call("cloud.dashboard", "get_top_file_types", [], { config_id: s.tft_drive_id });
                    if (!s.top_file_types_data) s.top_file_types_data = {};
                    s.top_file_types_data[s.tft_drive_id] = d;
                }
            },
            largest_files:    async () => { const d = await this.orm.call("cloud.dashboard", "get_largest_files", []); s.largest_files = d; },
            sync_trend:       async () => { const d = await this.orm.call("cloud.dashboard", "get_sync_trend_data", [], { start_date: s.globalTrendStart, end_date: s.globalTrendEnd }); s.trendData = d; },
            failed_syncs:     async () => { const d = await this.orm.call("cloud.dashboard", "get_error_summary", []); s.error_summary = d; },
            duplicates:       async () => { const d = await this.orm.call("cloud.dashboard", "get_duplicate_summary", []); s.duplicate_count = d.duplicate_count; },
            activity_log:     async () => { const d = await this.orm.call("cloud.dashboard", "get_activity_logs", [], { period: s.logFilter }); s.recent_logs = d; },
            recent_files: async () => {
                ...
                const records = await this.orm.searchRead(
                    ...
                    ['id', 'name', 'file_size', 'mime_type', 'cloud_url', 'cloud_file_id', 'sync_state', 'write_date']
                );
            ...
            active_shares:    async () => { 
                const d = await this.orm.call("cloud.dashboard", "get_active_shares", [], { 
                    config_id: s.asl_drive_id || false 
                }); 
                s.active_shares = d; 
            },
            top_uploaders:    async () => { const d = await this.orm.call("cloud.dashboard", "get_top_uploaders", []); s.top_uploaders = d; },
            model_breakdown:  async () => { const d = await this.orm.call("cloud.dashboard", "get_model_breakdown", []); s.model_breakdown = d; },
        };
    }
        ...
        return {
            ...
            storage_meter: el => {
                ...
                if (!q || (!q.available && !q.error)) {
                    ...
                    this.orm.call("cloud.dashboard", "get_drive_quota", [], { config_id: s.sm_drive_id })
            ...
            top_file_types: el => {
                ...
                if (!d) {
                    ...
                    this.orm.call("cloud.dashboard", "get_top_file_types", [], { config_id: s.tft_drive_id })
    ...
    async _filterLogs(period) {
        ...
        try {
            const logs = await this.orm.call("cloud.dashboard", "get_activity_logs", [], { period });
    ...
    _openSyncLogs() {
        this.action.doAction({ type: "ir.actions.act_window", res_model: "cloud.sync.log",
    ...
    _openDriveSettings(id) {
        this.action.doAction({ type: "ir.actions.act_window", res_model: "cloud.provider.config",
    ...
    openFileExplorer() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "cloud_storage_hub.file_explorer", name: "File Explorer" });
    }

    openActivityLogs() {
        this.action.doAction({ type: "ir.actions.client",
            tag: "cloud_storage_hub.sync_log_terminal", name: "Activity Log Terminal" });
    }
...
CloudHubDashboard.template = "cloud_storage_hub.Dashboard";
registry.category("actions").add("cloud_storage_hub.dashboard", CloudHubDashboard);
