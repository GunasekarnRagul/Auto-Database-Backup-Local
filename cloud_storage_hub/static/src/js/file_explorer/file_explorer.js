...
export class FileExplorer extends Component {
    setup() {
        ...
        if (this.busService) {
            ...
            this.busService.addChannel('cloud.sync.engine');
            this.busService.subscribe('notification', (notifications) => {
                notifications.forEach(notif => {
                    if (notif.type === 'cloud.sync.engine' && notif.payload.type === 'folder_synced') {
        ...
        onWillStart(async () => {
            ...
            if (section === 'trash') {
                ...
                if (this.state.currentFolderId) {
                    try {
                        const breadcrumbs = await this.orm.call("cloud.file", "get_folder_breadcrumbs", [this.state.currentFolderId]);
    ...
    async loadFiles(folderId = null) {
        ...
        let files = [];
        const commonFields = [
            "name", "file_type", "mime_type", "cloud_url", "file_size",
            "owner_name", "last_modified", "sync_state", "upload_progress", "starred",
            "drive_config_id", "cloud_file_id", "attachment_id", "display_path",
            "parent_folder_id"
        ];

        try {
            if (section === 'starred') {
                files = await this.orm.searchRead("cloud.file",
                ...
                if (recentIds.length === 0) {
                    ...
                    const recentFiles = await this.orm.searchRead("cloud.file",
                        domain,
                        commonFields
                    );
                ...
                if (folderId === 'trash_root' || !folderId) {
                    files = await this.orm.call("cloud.file", "get_trash_roots", []);
                } else {
                    files = await this.orm.searchRead("cloud.file",
                ...
                files = await this.orm.searchRead("cloud.file", domain, commonFields);
            }
    ...
    getFileImageUrl(file, isPreview = false) {
        ...
        const mime = file.mime_type || '';
        const basePath = '/cloud_storage_hub/static/description/';
    ...
    async executeSearch() {
        ...
        try {
            const files = await this.orm.searchRead("cloud.file", domain, [
                "name", "file_type", "mime_type", "cloud_url", "file_size",
                "owner_name", "last_modified", "sync_state", "upload_progress", "starred",
                "drive_config_id", "cloud_file_id", "attachment_id", "display_path",
                "parent_folder_id"
    ...
    onFileClick(ev, file) {
        ...
        if (file.file_type === 'folder') {
            ...
            if (file.cloud_file_id && !canSkipPreview) {
                // Previewable — open in dialog with iframe
                this.dialogService.add(FilePreviewDialog, {
                    file: file,
                    onDownload: () => {
                        window.open(`/cloud_hub/download/${file.id}`, '_blank');
                    },
                    onOpenDetails: () => {
                        this.actionService.doAction({
                            type: 'ir.actions.act_window',
                            res_model: 'cloud.file',
                            res_id: file.id,
                            views: [[false, 'form']],
                            target: 'new'
                        });
    ...
    async _loadTreeChildren(parentId, rootId = null) {
        ...
        try {
            ...
            const children = await this.orm.searchRead("cloud.file", domain, ["id", "name"]);
    ...
    async _updateNavigationState(folderId, folderName) {
        ...
        try {
            const breadcrumbs = await this.orm.call("cloud.file", "get_folder_breadcrumbs", [folderId]);
    ...
    async toggleStarSelected() {
        ...
        try {
            ...
            await this.orm.write("cloud.file", ids, { starred: newState });
    ...
    async toggleStar(ev, file) {
        ...
        try {
            await this.orm.write("cloud.file", [file.id], { starred: newState });
    ...
    async onRestoreSelected() {
        ...
        try {
            const ids = files.map(f => f.id);
            await this.orm.call("cloud.file", "action_unarchive", [ids]);
    ...
    async confirmPermanentDelete() {
        ...
        try {
            await this.orm.call("cloud.file", "delete_on_drive_and_unlink", [ids]);
            this.clearSelection();
            await this.onSidebarClick('trash');
    ...
    onDownloadSelected() {
        ...
        if (hasFolder || isMulti) {
            ...
            const downloadUrl = `/cloud_hub/download_zip?file_ids=${ids}`;
        ...
        const file = files[0];
        const downloadUrl = `/cloud_hub/download/${file.id}`;
    ...
    async onDeleteSelected() {
        ...
        try {
            ...
            await this.orm.call("cloud.file", "action_archive_recursive", [ids]);
    ...
    async confirmRename(file) {
        ...
        try {
            ...
            await this.orm.write("cloud.file", [fileId], {
                name: newName,
                sync_state: 'pending'
            });
            this.clearSelection();
            ...
            if (file.cloud_file_id) {
                try {
                    await this.orm.call("cloud.file", "rename_on_drive_by_id", [], {
                        record_id: fileId,
                        new_name: newName,
                        old_name: file.name,
                    });
    ...
    async checkPendingChanges() {
        if (!this.state.activeDriveId) return;
        const count = await this.orm.searchCount("cloud.file", [
    ...
    async confirmCreateFolder() {
        ...
        try {
            await this.orm.create("cloud.file", [{
                name: name,
                file_type: 'folder',
    ...
    async _uploadSingleFile(file, targetFolderId, targetRootId) {
        ...
        await this.orm.call("cloud.file", "action_upload_from_explorer", [], {
            file_name: file.name,
            file_data: base64,
    ...
    async onManualSync() {
        ...
        try {
            ...
            await this.orm.call("cloud.provider.config", "action_trigger_sync", [], {
     ...
     *
     * @param {number|null} driveId  cloud.provider.config ID to sync.
                ...
                for (const id of pendingIds) {
                    try {
                        ...
                        if (this.state.activeDriveId === targetDriveId) {
                            const updated = await this.orm.read("cloud.file", [id], [
                                "sync_state", "cloud_file_id", "cloud_url", "last_synced", "upload_progress"
                            ]);
...
FileExplorer.template = "cloud_storage_hub.FileExplorer";

registry.category("actions").add("cloud_storage_hub.file_explorer", FileExplorer);

class FilePreviewDialog extends Component {
    static template = "cloud_storage_hub.FilePreviewDialog";
    ...
    async loadPreviewUrl() {
        ...
        if (file.cloud_file_id) {
            try {
                ...
                const url = await this.orm.call("cloud.sync.engine", "get_preview_url", [
                    file.cloud_file_id, 
                    configId
                ]);
                if (url) {
...
export class ShareDriveLinkDialog extends Component {
    static template = "cloud_storage_hub.ShareDriveLinkDialog";
    static components = { Dialog };
    static props = {
        files: Array,
        close: Function,
    ...
    setup() {
        ...
        onWillStart(async () => {
            if (this.props.files.length === 1 && this.props.files[0].cloud_file_id) {
    ...
    get shareableFilesWithLinks() {
        return this.props.files
            .filter(f => f.cloud_file_id || f.file_type === 'folder')
            .map(f => ({
                ...f,
    ...
    async refreshPermissions() {
        if (this.state.isSyncing || !this.isSingleFile || !this.file.cloud_file_id) return;
    ...
    get shareableFiles() {
        return this.props.files.filter(f => f.cloud_url);
    }

    get shareLink() {
        const file = this.file;
        if (!file || !file.cloud_file_id) return '';
        // Prefer the presigned URL from the backend (state.shareLink) over
        // the plain S3 URL (cloud_url) which returns AccessDenied on private buckets.
        return this.state.shareLink || file.cloud_url || '';
    }
        ...
        try {
            ...
            if (result.error) {
                ...
                if (result.fallback_link) {
                    this.file.cloud_url = result.fallback_link;
                }
        ...
        try {
            ...
            if (result.error) {
                ...
                if (result.link && result.link.webUrl) {
                    this.file.cloud_url = result.link.webUrl;
                }
    ...
    async onCopyLink() {
        const link = this.shareLink || this.shareableFiles.map(f => f.cloud_url).join('\n');
        if (!link) return;
        try {
