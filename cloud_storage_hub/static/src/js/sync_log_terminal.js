...
export class SyncLogTerminal extends Component {
        ...
        try {
            const logs = await this.orm.searchRead(
                ...
                [
                    ...
                    "cloud_file_id", "file_size", "display_name", "folder_path"
                ],
            ...
            const successCount = await this.orm.searchCount("cloud.sync.log", sDomain);

            // 2. Fail Count for current filters
            const fDomain = statsDomain.filter(d => d[0] !== 'state');
            fDomain.push(['state', '=', 'fail']);
            const failCount = await this.orm.searchCount("cloud.sync.log", fDomain);
    ...
    async onClearLogs() {
        ...
        try {
            const allIds = await this.orm.search("cloud.sync.log", []);
            if (allIds.length) {
                await this.orm.unlink("cloud.sync.log", allIds);
            }
            await this.loadLogs();
        } catch (e) {
...
SyncLogTerminal.template = "cloud_storage_hub.SyncLogTerminal";

registry.category("actions").add("cloud_storage_hub.sync_log_terminal", SyncLogTerminal);
