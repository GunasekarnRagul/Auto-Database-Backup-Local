...
class DropboxFile(models.Model):
    ...
    _sql_constraints = [
        ('one_drive_file_id_drive_config_unique', 'unique(one_drive_file_id, drive_config_id)',
         'A file with this Dropbox ID already exists for this drive configuration.')
    ]
    ...
    def unlink(self):
        ...
        return super(DropboxFile, self).unlink()
    ...
    def write(self, vals):
        ...
        return super(DropboxFile, self).write(vals)
    ...
    def action_move_items(self, item_ids, target_parent_id=None, target_root_id=None):
        ...
        # Resolve target Dropbox file ID
    ...
    def rename_on_drive_by_id(self, record_id, new_name, old_name=None):
        ...
        if success:
            super(DropboxFile, record).write({
                'sync_state': 'synced',
                'last_synced': fields.Datetime.now(),
            })
        else:
