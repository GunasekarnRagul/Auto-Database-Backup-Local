...
class DropboxDashboard(models.AbstractModel):
    ...
    def get_kpi_data(self):
        ...
        try:
            ...
            # Get user's timezone for correct period boundaries
            user_tz = self.env.user.tz or 'UTC'
            import pytz
            tz = pytz.timezone(user_tz)
            now = fields.Datetime.now()
            now_user = pytz.utc.localize(now).astimezone(tz)
            
            start_of_today_user = now_user.replace(hour=0, minute=0, second=0, microsecond=0)
            today_start = start_of_today_user.astimezone(pytz.utc).replace(tzinfo=None)
            ...
            start_of_month_user = now_user.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            month_start = start_of_month_user.astimezone(pytz.utc).replace(tzinfo=None)
    ...
    def get_activity_logs(self, period='today', operation=False, limit=1000):
        ...
        try:
            ...
            if period == 'today':
                ...
                domain.append(('create_date', '>=', start_of_today_utc))
            elif period == '7d':
                start_of_period_user = (now_user - timedelta(days=7)).replace(hour=0, minute=0, second=0, microsecond=0)
                start_of_period_utc = start_of_period_user.astimezone(pytz.utc).replace(tzinfo=None)
                domain.append(('create_date', '>=', start_of_period_utc))
            elif period == '30d':
                start_of_period_user = (now_user - timedelta(days=30)).replace(hour=0, minute=0, second=0, microsecond=0)
                start_of_period_utc = start_of_period_user.astimezone(pytz.utc).replace(tzinfo=None)
                domain.append(('create_date', '>=', start_of_period_utc))
    ...
    def get_sync_trend_data(self, days=7, start_date=False, end_date=False):
        ...
        try:
            ...
            self.env.cr.execute("""
                SELECT
                    TO_CHAR(create_date AT TIME ZONE 'UTC' AT TIME ZONE %s, 'YYYY-MM-DD') AS sync_date,
                    state,
                    COUNT(*)                                              AS cnt
