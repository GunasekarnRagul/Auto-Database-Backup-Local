...
export class SyncLogTerminal extends Component {
        ...
        if (this.busService) {
            ...
            this.busService.subscribe('notification', (notifications) => {
                for (const notif of notifications) {
                    if (notif.type === 'one.drive.sync') {
                        const payload = notif.payload;
                        if (payload && payload.type === 'sync_log_updated') {
                            this.loadLogs();
                        }
                    }
                }
        ...
        try {
            const logs = await this.orm.searchRead(
                ...
                [
                    ...
                    "one_drive_file_id", "file_size", "display_name", "folder_path"
                ],
                { limit: this.state.limit, order: "create_date desc, id desc" }
            );
            ...
            const successCount = await this.orm.searchCount("one.drive.sync.log", sDomain);

            // 2. Fail Count for current filters
            const fDomain = statsDomain.filter(d => d[0] !== 'state');
            fDomain.push(['state', '=', 'fail']);
            const failCount = await this.orm.searchCount("one.drive.sync.log", fDomain);
    ...
    async onClearLogs() {
        ...
        try {
            const allIds = await this.orm.search("one.drive.sync.log", []);
            if (allIds.length) {
                await this.orm.unlink("one.drive.sync.log", allIds);
            }
            await this.loadLogs();
        } catch (e) {
...
SyncLogTerminal.template = "dropbox_odoo_integration.SyncLogTerminal";

registry.category("actions").add("dropbox_odoo_integration.sync_log_terminal", SyncLogTerminal);
