...
export class FileExplorer extends Component {
    setup() {
        ...
        if (this.busService) {
            ...
            this.busService.addChannel('one.drive.sync');
            this.busService.subscribe('notification', (notifications) => {
                notifications.forEach(notif => {
                    if (notif.type === 'one.drive.sync' && notif.payload.type === 'folder_synced') {
    ...
    async loadFiles(folderId = null) {
        ...
        try {
            if (section === 'starred') {
                files = await this.orm.searchRead("one.drive.file",
                ...
                if (recentIds.length === 0) {
                    ...
                    const recentFiles = await this.orm.searchRead("one.drive.file",
                        domain,
                        commonFields
                    );
                ...
                if (folderId === 'trash_root' || !folderId) {
                    files = await this.orm.call("one.drive.file", "get_trash_roots", []);
                } else {
                    files = await this.orm.searchRead("one.drive.file",
                ...
                files = await this.orm.searchRead("one.drive.file", domain, commonFields);
            }
    ...
    getFileImageUrl(file, isPreview = false) {
        ...
        const mime = file.mime_type || '';
        const basePath = '/dropbox_odoo_integration/static/description/';
    ...
    async executeSearch() {
        ...
        try {
            const files = await this.orm.searchRead("one.drive.file", domain, [
                "name", "file_type", "mime_type", "one_drive_url", "file_size",
                "owner_name", "last_modified", "sync_state", "upload_progress", "starred",
                "drive_config_id", "one_drive_file_id", "attachment_id", "display_path",
                "parent_folder_id"
    ...
    onFileClick(ev, file) {
        ...
        if (file.file_type === 'folder') {
            ...
            // Dropbox's previewer handles virtually all formats natively (PDF, Office,
            ...
            if (file.one_drive_file_id && !canSkipPreview) {
                this.dialogService.add(FilePreviewDialog, {
                    file: file,
                    onDownload: () => {
                        window.open(`/dropbox/download/${file.id}`, '_blank');
                    },
                    onOpenDetails: () => {
                ...
                // If not previewable or missing Dropbox ID, open form
                this.actionService.doAction({
                    type: 'ir.actions.act_window',
                    res_model: 'one.drive.file',
                    res_id: file.id,
    ...
    async onPaste() {
        ...
        if (this.state.activeDriveId !== this.state.clipboard.sourceDriveId) {
            this.notificationService.add("Other drive not pasted. Moving items across different Dropboxs is not supported.", { 
                type: "danger",
                sticky: true 
            });
            return;
    ...
    async _loadTreeChildren(parentId, rootId = null) {
        ...
        try {
            ...
            const children = await this.orm.searchRead("one.drive.file", domain, ["id", "name"]);
    ...
    async _updateNavigationState(folderId, folderName) {
        ...
        try {
            const breadcrumbs = await this.orm.call("one.drive.file", "get_folder_breadcrumbs", [folderId]);
    ...
    async toggleStarSelected() {
        ...
        try {
            ...
            await this.orm.write("one.drive.file", ids, { starred: newState });
    ...
    async toggleStar(ev, file) {
        ...
        try {
            await this.orm.write("one.drive.file", [file.id], { starred: newState });
    ...
    async onRestoreSelected() {
        ...
        try {
            const ids = files.map(f => f.id);
            await this.orm.call("one.drive.file", "action_unarchive", [ids]);
    ...
    async confirmPermanentDelete() {
        ...
        try {
            await this.orm.call("one.drive.file", "delete_on_drive_and_unlink", [ids]);
            this.clearSelection();
            await this.onSidebarClick('trash');
    ...
    onDownloadSelected() {
        ...
        if (hasFolder || isMulti) {
            ...
            const downloadUrl = `/dropbox/download_zip?file_ids=${ids}`;
        ...
        const file = files[0];
        const downloadUrl = `/dropbox/download/${file.id}`;
        const a = document.createElement('a');
        a.style.display = 'none';
        a.href = downloadUrl;
    ...
    async onDeleteSelected() {
        ...
        try {
            ...
            await this.orm.call("one.drive.file", "action_archive_recursive", [ids]);
            ...
            this.notificationService.add(
                `Moved to Odoo Trash: ${nameList}. These items will remain active on Dropbox until permanently deleted from the Trash tab.`,
                { type: "info" }
            );
            await this.checkPendingChanges();
        } catch (e) {
    ...
    async confirmRename(file) {
        ...
        try {
            ...
            await this.orm.write("one.drive.file", [fileId], {
                name: newName,
                sync_state: 'pending'
            });
            this.clearSelection();
            ...
            if (file.one_drive_file_id) {
                try {
                    await this.orm.call("one.drive.file", "rename_on_drive_by_id", [], {
                        record_id: fileId,
                        new_name: newName,
                        old_name: file.name,
                    });
                    this.loadFiles(this.state.currentFolderId); // Refresh to clear 'pending'
                    this.notificationService.add(`Renamed to "${newName}" on Dropbox`, { type: "success" });
                } catch (e) {
                    this.notificationService.add(`Failed to rename "${newName}" on Dropbox`, { type: "warning" });
                }
            } else {
    ...
    async checkPendingChanges() {
        if (!this.state.activeDriveId) return;
        const count = await this.orm.searchCount("one.drive.file", [
    ...
    async confirmCreateFolder() {
        ...
        try {
            await this.orm.create("one.drive.file", [{
                name: name,
                file_type: 'folder',
    ...
    async _uploadSingleFile(file, targetFolderId, targetRootId) {
        ...
        await this.orm.call("one.drive.file", "action_upload_from_explorer", [], {
            file_name: file.name,
            file_data: base64,
    ...
    async onManualSync() {
        ...
        try {
            ...
            await this.orm.call("one.drive.config", "action_trigger_sync", [], {
    ...
    /**
     * Trigger a one-way auto-sync (Odoo → Dropbox) for a specific drive.
     *
     ...
     *
     * @param {number|null} driveId  one.drive.config ID to sync.
                ...
                for (const id of pendingIds) {
                    try {
                        ...
                        if (this.state.activeDriveId === targetDriveId) {
                            const updated = await this.orm.read("one.drive.file", [id], [
                                "sync_state", "one_drive_file_id", "one_drive_url", "last_synced", "upload_progress"
                            ]);
...
FileExplorer.template = "dropbox_odoo_integration.FileExplorer";

registry.category("actions").add("dropbox_odoo_integration.file_explorer", FileExplorer);

class FilePreviewDialog extends Component {
    static template = "dropbox_odoo_integration.FilePreviewDialog";
        ...
        if (file.one_drive_file_id) {
            try {
                ...
                if (url) {
                    ...
                    this.state.error = "Dropbox could not generate a preview for this file type.";
                }
            } catch (e) {
    ...
    get openInDriveUrl() {
        const { file } = this.props;
        return file.one_drive_url;
    }
}

export class ShareDriveLinkDialog extends Component {
    static template = "dropbox_odoo_integration.ShareDriveLinkDialog";
    static components = { Dialog };
    static props = {
        files: Array,
        close: Function,
    ...
    _startAutoSync() {
        ...
        // Poll Dropbox every 30s while the dialog is open
    ...
    get shareableFiles() {
        return this.props.files.filter(f => f.one_drive_url);
    }

    get shareLink() {
        const file = this.file;
        if (!file || !file.one_drive_file_id) return '';
        return file.one_drive_url || '';
    }
    ...
    async loadShareInfo() {
        ...
        try {
            ...
            if (result.error) {
                ...
                const now = new Date();
                this.state.lastSyncTime = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                // Start auto-polling so Dropbox-side changes reflect here every 30s
                this._startAutoSync();
            }
        } catch (e) {
